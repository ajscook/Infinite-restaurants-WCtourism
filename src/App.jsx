  ...
  <section style={{ display: "flex", flexDirection: "column", justifyContent: "flex-start", padding: "clamp(40px,8vh,100px) 0 clamp(24px,4vh,48px)", position: "relative", overflow: "hidden" }}>
    ...
    <div style={{ ...W, position: "relative", zIndex: 1, padding: "48px 120px 18px" }}>
      ...
      <h1 className="d u1" style={{ fontSize: "clamp(28px,5vw,64px)", fontWeight: 800, lineHeight: 1.0, letterSpacing: "-.03em", marginBottom: 28, color: C.text }}>
        <span style={{ color: C.orange }}>Forget</span><span style={{ color: C.text }}>: </span><span style={{ color: C.blue }}>Soccer</span><span style={{ color: C.text }}> in Stadiums.</span><br />
        <span style={{ color: C.blue }}>Think</span><span style={{ color: C.text }}>: </span><span style={{ color: C.orange }}>Tourism Surge </span><span style={{ color: C.text }}>Between Matches.</span>
      </h1>
        ...
        <div onClick={() => setBriefOpen(false)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.78)", zIndex:500, display:"flex", alignItems:"center", justifyContent:"center", padding:24, backdropFilter:"blur(4px)", animation:"in .18s ease" }}>
          <div onClick={e => e.stopPropagation()} style={{ background:C.surface, border:`2px solid ${C.blue}`, borderRadius:14, width:"100%", maxWidth:720, maxHeight:"88vh", display:"flex", flexDirection:"column", boxShadow:"0 24px 64px rgba(0,0,0,.6)", overflow:"hidden" }}>
            ...
            <div style={{ padding:"16px 24px", borderTop:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0, background:C.surface }}>
              <button onClick={() => setBriefStep(s => Math.max(1, s-1))} disabled={briefStep===1} style={{ background:"none", border:`1px solid ${briefStep===1?C.border:C.dim}`, borderRadius:20, padding:"7px 18px", cursor:briefStep===1?"default":"pointer", fontSize:13, fontWeight:700, letterSpacing:".08em", textTransform:"uppercase", color:briefStep===1?C.border:C.muted, fontFamily:"'Plus Jakarta Sans',sans-serif", transition:"all .15s" }}>
                Back
              </button>
...
const SAction = () => {
  ...
  return (
    <Sec id="action" num="04" label="Prep Priorities" accent={C.orange}>
      <h2 className="d" style={{ fontSize:"clamp(28px,4.2vw,48px)", fontWeight:800, lineHeight:1.04, letterSpacing:"-.02em", marginBottom:20, color:C.text, textTransform:"uppercase" }}>
        We can't be 'ready', but we can be 'readier' and keep our knees bent
      </h2>
      <p style={{ fontSize:19, color:C.muted, fontWeight:300, lineHeight:1.78, marginBottom:12, maxWidth: "90%" }}>
        Food, labor, operations, tech. You run these four things every shift. What's different is who's walking in, how they found you, and what happens to your staffing and supply chain while they're here.
      </p>
      <p style={{ fontSize:19, color:C.muted, fontWeight:300, lineHeight:1.78, marginBottom:12, maxWidth: "90%" }}>
        Millions more on the roads moving through North America for over a month puts pressure on all four at the same time. Your staff, shelves, hours, tech may not be ready for this. The cards below are organized around where the pressure actually lands. Start with the one that makes you more nervous than the others.
      </p>
