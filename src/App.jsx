  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.82)", zIndex:600, display:"flex", alignItems:"flex-start", justifyContent:"center", padding:"24px", backdropFilter:"blur(6px)", animation:"in .18s ease", overflowY:"auto" }}>
      <div onClick={e => e.stopPropagation()} style={{ background:C.surface, border:`2px solid ${C.blue}`, borderRadius:16, width:"100%", maxWidth:760, marginTop:8, marginBottom:24, boxShadow:"0 32px 80px rgba(0,0,0,.7)", display:"flex", flexDirection:"column" }}>
        ...
        {/* ChatGPT: unique layout. Claude/Gemini: tabbed layout */}
        {model.id === "chatgpt" ? (
          <div style={{ padding:"24px", overflowY:"auto", maxHeight:"60vh", display:"flex", flexDirection:"column", gap:20 }}>

            {/* Handoff prompt */}
            <div>
              <div style={{ fontSize:12, fontWeight:700, letterSpacing:".14em", textTransform:"uppercase", color:C.blue, marginBottom:6 }}>Your Handoff Prompt</div>
              <p style={{ fontSize:14, color:C.muted, lineHeight:1.7, marginBottom:10 }}>
                The Custom GPT already has the research. This gives it you. Copy it, open the Custom GPT, paste it in. It takes it from there.
              </p>
              {!formReady && <p style={{ fontSize:13, color:C.orange, fontStyle:"italic", marginBottom:10 }}>Add your address and website in Step 1 to generate your handoff.</p>}
              {formReady && (
                <>
                  <PromptBox value={shortInput} minH={100}/>
                  <div style={{ display:"flex", gap:10, marginTop:12 }}>
                    ...
                    <span style={{ fontSize:12, color:C.dim, alignSelf:"center" }}>Custom GPT coming soon</span>
                  </div>
                </>
              )}
            </div>
            ...
            <div style={{ borderTop:`1px solid ${C.border}`, paddingTop:20 }}>
              <div style={{ fontSize:12, fontWeight:700, letterSpacing:".14em", textTransform:"uppercase", color:C.blue, marginBottom:14 }}>Focused Prompts for the Custom GPT</div>
              {pillarGroups.map(pg => (
    ...
    <Sec id="ai" num="05" label="HI ∞ AI ∞ HI Prompts" accent={C.blue}>
      ...
      <div className="card-blue" style={{ background:C.surface, borderRadius:12, padding:"24px 24px 20px", marginBottom:20 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:14, marginBottom:16 }} className="three-col">
          ...
          <div>
            <div style={{ fontSize:13, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", color:C.dim, marginBottom:6 }}>Google Business Profile URL</div>
