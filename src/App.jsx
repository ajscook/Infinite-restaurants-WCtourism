    ...
    <nav style={{ position: "sticky", top: 0, zIndex: 200, background: "rgba(13,15,20,.96)", backdropFilter: "blur(16px)", borderBottom: `1px solid ${C.border}` }}>
      <div style={{ ...W, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "26px 120px", position: "relative" }}>
        ...
        <div className="nav-tabs-desktop">
          {[["who",<>Super<br/>Tourist</>],["where",<>Travel<br/>Patterns</>],["position",<>Your<br/>Position</>],["action",<>Prep<br/>Priorities</>],["offramp",<>AI<br/>Offramp</>]].map(([id, label]) => (
...
const buildContextProfile = (place) => {
  ...
  const attrs = [];
  if (place.dineIn != null) attrs.push(`dine-in: ${place.dineIn}`);
  if (place.takeout != null) attrs.push(`takeout: ${place.takeout}`);
  if (place.delivery != null) attrs.push(`delivery: ${place.delivery}`);
  if (place.reservable != null) attrs.push(`reservable: ${place.reservable}`);
  if (place.servesBreakfast != null) attrs.push(`breakfast: ${place.servesBreakfast}`);
  if (place.servesLunch != null) attrs.push(`lunch: ${place.servesLunch}`);
  if (place.servesDinner != null) attrs.push(`dinner: ${place.servesDinner}`);
  if (place.servesBrunch != null) attrs.push(`brunch: ${place.servesBrunch}`);
  if (place.servesBeer != null) attrs.push(`beer: ${place.servesBeer}`);
  if (place.servesWine != null) attrs.push(`wine: ${place.servesWine}`);
  if (place.servesCocktails != null) attrs.push(`cocktails: ${place.servesCocktails}`);
  if (place.outdoorSeating != null) attrs.push(`outdoor seating: ${place.outdoorSeating}`);
  if (place.liveMusic != null) attrs.push(`live music: ${place.liveMusic}`);
  if (place.goodForGroups != null) attrs.push(`good for groups: ${place.goodForGroups}`);
  if (place.goodForWatchingSports != null) attrs.push(`good for watching sports: ${place.goodForWatchingSports}`);
  if (attrs.length) {
    lines.push(``);
...
const aiOfframpPlatforms = {
  ...
  notebooklm: {
    name: "NotebookLM",
    displayName: "Notebook LM",
    fallback: "Nb",
    tagline: "A notebook base to build on",
    accent: C.blue,
    cardClass: "card-blue",
    openLabel: "Open in NotebookLM",
    openUrl: "https://notebooklm.google.com",
    blurb: "Paste the context profile into a NotebookLM notebook alongside the Infinite Restaurants research so you have a base to build on.",
  },
  claude: {
    name: "Claude",
    displayName: "Claude",
    fallback: "Cl",
    tagline: "Custom Prompt Generators",
    accent: C.orange,
    cardClass: "card-orange",
    openLabel: "Open in Claude",
    openUrl: "https://claude.ai",
    blurb: "Paste the context profile into Claude so it can think alongside you with the same research this platform is built on.",
  },
};
    ...
    try {
      ...
      await place.fetchFields({
        fields: [
          ...
          "outdoorSeating","liveMusic","goodForGroups","goodForWatchingSports",
        ],
      });
    ...
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.72)", zIndex:500, display:"flex", alignItems:"center", justifyContent:"center", padding:"24px", backdropFilter:"blur(4px)", animation:"in .18s ease" }}>
      <div onClick={e => e.stopPropagation()} style={{ background:C.surface, border:`2px solid ${p.accent}`, borderRadius:14, width:"100%", maxWidth:780, maxHeight:"85vh", overflow:"auto", boxShadow:"0 24px 64px rgba(0,0,0,.5)" }}>
        ...
        <div style={{ padding:"24px" }}>
          ...
          {place && (
            <>
              <div style={{ marginTop:24, fontSize:13, fontWeight:700, letterSpacing:".12em", textTransform:"uppercase", color:C.text, marginBottom:10 }}>2. Context profile</div>
...
const OfframpTile = ({ platformKey, onOpen }) => {
  const p = aiOfframpPlatforms[platformKey];
  return (
    <div className={p.cardClass} onClick={() => onOpen(platformKey)}
      style={{ background:C.surface, borderRadius:12, padding:"24px 20px", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"flex-start", textAlign:"center", height:"100%", minHeight:140 }}>
...
const SOfframp = () => {
  ...
  <Sec id="offramp" num="05" label="AI Offramp" accent={C.orange}>
    ...
    <p style={{ fontSize:16, color:C.muted, fontWeight:300, lineHeight:1.78, marginBottom:20, maxWidth:"90%" }}>
      You already use an AI that knows your operation. Building a chatbot here would hand you one that doesn't. The tiles below bring this research into yours instead.
    </p>
    <p style={{ fontSize:16, color:C.muted, fontWeight:300, lineHeight:1.78, marginBottom:24, maxWidth:"90%" }}>
      Find your restaurant. Generate a structured handoff. Open it in the AI you already work with. Your instinct runs the conversation. The research rides along.
    </p>
    <div className="callout-left-orange" style={{ marginBottom:32 }}>
    ...
    <div className="offramp-grid" style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:20, alignItems:"stretch" }}>
      <OfframpTile platformKey="chatgpt" onOpen={setModal} />
      <OfframpTile platformKey="gemini" onOpen={setModal} />
      <OfframpTile platformKey="notebooklm" onOpen={setModal} />
