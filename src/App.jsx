  ...
  useEffect(() => {
    ...
    if (document.getElementById("gmp-script")) {
      const check = setInterval(() => {
        if (window.google?.maps?.places) { setReady(true); clearInterval(check); }
      }, 100);
      return () => clearInterval(check);
    }
    const s = document.createElement("script");
    s.id = "gmp-script";
    s.src = `https://maps.googleapis.com/maps/api/js?key=${PLACES_API_KEY}&libraries=places`;
    s.async = true;
...
// ─── FORMAT HOURS FROM PLACES API ────────────────────────────────────────────
const formatPlacesHours = (periods) => {
  ...
  const byDay = {};
  periods.forEach(p => {
    const d = p.open?.day ?? p.open?.dayOfWeek;
    if (d == null) return;
    const open = fmt(p.open?.time ?? (p.open?.hours != null ? p.open.hours * 100 + (p.open.minutes ?? 0) : null));
    const close = fmt(p.close?.time ?? (p.close?.hours != null ? p.close.hours * 100 + (p.close.minutes ?? 0) : null));
...
const PlacesSearch = ({ onSelect }) => {
  ...
  const search = (val) => {
    ...
    svc.getPlacePredictions({
      ...
      types: ["restaurant","bar","food","cafe"],
    }, (predictions, status) => {
      setLoading(false);
  ...
  const handleSelect = (prediction) => {
    ...
    svc.getDetails({
      ...
      sessionToken: sessionRef.current,
      fields: [
        "name","formatted_address","website","url",
      ...
      if (status === window.google.maps.places.PlacesServiceStatus.OK && place) {
        const hours = formatPlacesHours(place.opening_hours?.periods);
        ...
        <div style={{ position:"absolute", top:"calc(100% + 4px)", left:0, right:0, background:C.surface, border:`1px solid ${C.blue}`, borderRadius:8, zIndex:400, overflow:"hidden", boxShadow:"0 8px 32px rgba(0,0,0,.4)" }}>
          {suggestions.map((s, i) => (
            <button key={s.place_id} onMouseDown={() => handleSelect(s)}
              ...
              <div style={{ fontSize:14, fontWeight:700, color:C.text, marginBottom:2 }}>{s.structured_formatting?.main_text || s.description}</div>
              <div style={{ fontSize:12, color:C.dim }}>{s.structured_formatting?.secondary_text || ""}</div>
            </button>
          ))}
        </div>
      )}
