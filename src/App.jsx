...
const buildContextProfile = (place) => {
  ...
  lines.push(`Generated from Google Places for: ${place.displayName || place.name || "(unnamed)"}`);
  lines.push(``);
  if (place.formattedAddress) lines.push(`Address: ${place.formattedAddress}`);
  if (place.location?.lat && place.location?.lng) lines.push(`Coordinates: ${place.location.lat}, ${place.location.lng}`);
  ...
  if (place.rating) lines.push(`Rating: ${place.rating}${place.userRatingCount ? ` (${place.userRatingCount} reviews)` : ""}`);
  ...
  if (place.businessStatus) lines.push(`Business status: ${place.businessStatus}`);
  ...
  lines.push(``);
  lines.push(`---`);
  lines.push(`Use this profile to personalize any Infinite Restaurants research output for this operation. The operator's instinct is the first and last gate on every decision.`);
  return lines.join("\n");
};
...
const aiOfframpPlatforms = {
  ...
  gemini: {
    ...
    openUrl: "https://gemini.google.com/gem/1_HdxHgO8xaGyHB32EfEfRT6J-i9fCxtk?usp=sharing",
  ...
  const inner = (
    <>
      <div style={{ width:56, height:56, flexShrink:0, borderRadius:10, background:C.surf2, border:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden", position:"relative" }}>
        <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, fontWeight:800, color:p.accent, fontFamily:"'Bricolage Grotesque',sans-serif", letterSpacing:"-.02em" }}>{p.fallback}</div>
        <img src={`https://infiniterestaurants.ai/${platformKey}.png`} alt={p.name} style={{ width:"100%", height:"100%", objectFit:"contain", display:"block", position:"relative", zIndex:1 }} onError={e => { e.currentTarget.style.display="none"; }} />
      </div>
...
const SOfframp = () => {
  ...
  <Sec id="offramp" num="05" label="AI Offramp" accent={C.orange}>
    ...
    <p style={{ fontSize:16, color:C.muted, fontWeight:300, lineHeight:1.78, marginBottom:24, maxWidth:"90%" }}>
      Pick the tile that matches your platform. Ask the questions only you would ask. Pull any thread, bring in your own context, push back when something feels off.
    </p>
    <div className="offramp-grid" style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:20, alignItems:"stretch", marginBottom:32 }}>
    ...
    {modal && <OfframpModal platformKey={modal} onClose={() => setModal(null)} />}
