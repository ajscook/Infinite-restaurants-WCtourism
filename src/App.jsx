...
const FontLoader = () => (
  <style>{`
    @import url('https://fonts.googleapis.com/css2?family=Bricolage+Grotesque:opsz,wght@12..96,400;12..96,600;12..96,700;12..96,800&family=Oswald:wght@400;500;600;700&family=Jost:ital,wght@0,300;0,400;0,500;0,600;1,400&display=swap');
  `}</style>
);
...
const G = () => (
  <style>{`
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    body{background:${C.bg};color:${C.text};font-family:'Jost',sans-serif;-webkit-font-smoothing:antialiased;line-height:1.65;}
    ...
    .caps{font-family:'Jost',sans-serif;font-size:11px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:${C.text};}
    /* ALL CAPS DIM - nav/section labels */
    .caps-dim{font-family:'Jost',sans-serif;font-size:11px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:${C.dim};}

    .nav-link{background:none;border:none;font-family:'Jost',sans-serif;font-size:13px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:${C.muted};padding:8px 10px;border-radius:6px;cursor:pointer;transition:color .18s;white-space:normal;text-align:center;line-height:1.3;}
    ...
    .ticker-wrap{background:${C.blue};overflow:hidden;padding:11px 0;border-top:1px solid ${C.orange};border-bottom:1px solid ${C.orange};}
...
const SecLabel = ({ children, color = C.dim }) => (
  <div style={{ fontFamily: "'Jost',sans-serif", fontSize: 16, fontWeight: 700, letterSpacing: ".14em", textTransform: "uppercase", color: C.text, marginBottom: 20, display: "flex", alignItems: "center", gap: 8 }}>
    <div style={{ width: 48, height: 48, borderRadius: 8, background: "transparent", flexShrink: 0, overflow: "hidden" }}>
      <img src="/IR_Mini.png" alt="" style={{ width: 48, height: 48, objectFit: "contain", display: "block" }} onError={e => { e.currentTarget.style.display = "none"; }} />
    </div>
...
const Badge = ({ children, color = C.dim }) => (
  <span style={{ display: "inline-flex", alignItems: "center", fontFamily: "'Jost',sans-serif", fontSize: 14, fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", padding: "4px 12px", borderRadius: 20, border: `1px solid ${color}`, color }}>
    {children}
  </span>
);
  ...
  return (
    <div style={{ borderBottom: `1px solid ${C.border}` }}>
      <button onClick={() => setOpen(o => !o)} style={{ width: "100%", background: "none", border: "none", cursor: "pointer", padding: "24px 0", textAlign: "left", display: "flex", alignItems: "flex-start", gap: 16 }}>
        ...
        <div style={{ width: 32, height: 32, flexShrink: 0, borderRadius: 6, border: `2px solid ${open ? accent : C.border}`, display: "flex", alignItems: "center", justifyContent: "center", transition: "transform .3s,border-color .2s", transform: open ? "rotate(45deg)" : "none", color: open ? accent : C.muted, marginTop: 3 }}>
...
const Nav = ({ active }) => {
  ...
  return (
    <nav style={{ position: "sticky", top: 0, zIndex: 200, background: "rgba(13,15,20,.96)", backdropFilter: "blur(16px)", borderBottom: `1px solid ${C.border}` }}>
      <div style={{ ...W, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "16px 120px", position: "relative" }}>
        <Logo />

        {/* Desktop tabs */}
        <div className="nav-tabs-desktop">
          {[["who",<>Super<br/>Tourist</>],["where",<>Travel<br/>Patterns</>],["position",<>Your<br/>Position</>],["action",<>Prep<br/>Priorities</>]].map(([id, label]) => (
  ...
  <section style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", position: "relative", overflow: "hidden" }}>
    ...
    <div style={{ ...W, position: "relative", zIndex: 1, padding: "48px 120px 18px" }}>
      ...
      <h1 className="d u1" style={{ fontSize: "clamp(28px,5vw,64px)", fontWeight: 800, lineHeight: 1.0, letterSpacing: "-.03em", marginBottom: 28, color: C.text }}>
        <span style={{ color: C.text }}>This Isn't About </span><span style={{ color: C.blue }}>Soccer in Stadiums</span><br />
        <span style={{ color: C.text }}>It's the </span><span style={{ color: C.orange }}>Tourism Surge </span><span style={{ color: C.text }}>Between Matches</span>
      </h1>
      ...
      <div className="u2" style={{ marginBottom: 48, maxWidth: "90%" }}>
        ...
        <p style={{ fontSize: "clamp(17px,2vw,20px)", fontWeight: 700, color: C.text, lineHeight: 1.6, marginBottom: 14, textTransform: "uppercase", letterSpacing: ".06em" }}>
          Three tournaments of data show the same pattern:
        </p>
      ...
      <div className="u3" style={{ display:"flex", gap:14, alignItems:"center", marginBottom:48, flexWrap:"wrap" }}>
        <button
          onClick={() => document.getElementById("about")?.scrollIntoView({ behavior:"smooth" })}
          style={{ background:C.orange, color:"#fff", border:"none", borderRadius:8, cursor:"pointer", padding:"15px 32px", fontFamily:"'Jost',sans-serif", fontSize:14, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", transition:"opacity .2s", whiteSpace:"nowrap" }}
          onMouseEnter={e => e.currentTarget.style.opacity=".82"}
          onMouseLeave={e => e.currentTarget.style.opacity="1"}
        >
          Who Built This <svg width="10" height="10" viewBox="0 0 10 10" fill="none" style={{display:"inline",verticalAlign:"middle",marginLeft:4}}><polygon points="2,1 9,5 2,9" fill="currentColor"/></svg>
        </button>
        <button
          onClick={onContinue}
          style={{ background:C.blue, color:"#fff", border:"none", borderRadius:8, cursor:"pointer", padding:"15px 32px", fontFamily:"'Jost',sans-serif", fontSize:14, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", transition:"opacity .2s", whiteSpace:"nowrap" }}
          onMouseEnter={e => e.currentTarget.style.opacity=".82"}
          onMouseLeave={e => e.currentTarget.style.opacity="1"}
        >
          See the research <svg width="10" height="10" viewBox="0 0 10 10" fill="none" style={{display:"inline",verticalAlign:"middle",marginLeft:4}}><polygon points="1,2 9,2 5,9" fill="currentColor"/></svg>
        </button>
      </div>
      <div className="u5 hero-stats card-blue" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", alignItems: "center", marginTop: 0, borderRadius: 12, overflow: "hidden", background: C.surface }}>
        {[
          ...
          { n:"80 Mi", l:"Excursion Range",  c:C.orange },
        ].map((s,i) => (
      ...
      <div onClick={e => e.stopPropagation()} style={{ background:C.surface, border:`1px solid ${C.blue}`, borderRadius:14, maxWidth:560, width:"100%", overflow:"hidden", boxShadow:"0 24px 64px rgba(0,0,0,.5)" }}>
        ...
        <div style={{ padding:"24px" }}>
          ...
          {content.body.map((p,i) => (
            <p key={i} style={{ fontSize:16, color:C.muted, lineHeight:1.78, marginBottom: i < content.body.length-1 ? 14 : 0 }}>{p}</p>
          ))}
        </div>
...
const SWho = () => {
  ...
  <Sec id="who" num="01" label="The Super Tourist" accent={C.blue}>
    <h2 className="d" style={{ fontSize: "clamp(28px,4.2vw,48px)", fontWeight: 800, lineHeight: 1.04, letterSpacing: "-.02em", marginBottom: 18, color: C.text }}>
      Meet the <span style={{ color: C.orange }}>Super</span> <span style={{ color: C.blue }}>Tourist</span>
    </h2>
    ...
    <p style={{ fontSize: 19, fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", color: C.text, marginBottom: 6, maxWidth: "90%" }}>
      The host cities cannot contain them.
    </p>
    <p style={{ fontSize: 19, color: C.muted, fontWeight: 300, lineHeight: 1.78, marginBottom: 10, maxWidth: "90%" }}>
      Your town could be along their route whether you're planning for it or not.
    </p>
    <p style={{ fontSize: 19, color: C.text, fontWeight: 700, lineHeight: 1.78, marginBottom: 24, maxWidth: "90%" }}>
      And we're going to be feeding and serving them for a month.
    </p>
    ...
    <div className="card-blue" style={{ background: C.surface, borderRadius: 12, padding: "22px 28px", marginBottom: 20 }}>
      <div className="multiplier-bar" style={{ display: "flex", alignItems: "center" }}>
        ...
        <div style={{ paddingLeft: 24, flexShrink: 0, maxWidth: 220 }}>
          <p style={{ fontSize: 16, fontWeight: 700, color: C.text, lineHeight: 1.5 }}>
    ...
    <div className="tourist-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 4 }}>
      ...
      <div className="card-orange" style={{ borderRadius: 12, overflow: "hidden" }}>
        <div style={{ padding: "22px 24px 20px", borderBottom: `1px solid ${C.border}` }}>
          ...
          <p style={{ fontSize: 16, color: C.muted, lineHeight: 1.72, marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 700, color: C.text, marginBottom: 4 }}>International Football Visitor</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
          {[
            ...
            <button key={i} onClick={() => setModal(s.k)} style={{ padding: "16px 18px", borderTop: `1px solid ${C.border}`, borderRight: i%2===0?`1px solid ${C.border}`:"none", borderBottom:"none", borderLeft:"none", background:"none", cursor:"pointer", textAlign:"left", position:"relative", transition:"background .15s" }}
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                ...
                <svg width="22" height="22" viewBox="0 0 22 22" fill="none"><polygon points="4,3 19,11 4,19" fill={s.c}/></svg>
              </div>
      ...
      <div className="card-dim" style={{ borderRadius: 12, overflow: "hidden" }}>
        <div style={{ padding: "22px 24px 20px", borderBottom: `1px solid ${C.border}` }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 700, color: C.muted, marginBottom: 4 }}>Standard Summer Visitor</div>
...
const supplyCards = [
  { id:"rep", letter:"R", label:"Sales Rep Convos", sub:"Coordinate before the pressure arrives", color:C.blue,
    preview:["Discuss your delivery corridor","Ask about alternatives: days, windows, splits","Show them the chain if they're skeptical"],
  ...
  { id:"regional", letter:"D", label:"Local Distributor", sub:"Your backup. More nimble, shorter routes", color:C.blue,
    preview:["Get your credit app in now","Start accepting deliveries before June","Your business means more to them proportionally"],
  ...
  { id:"menu", letter:"M", label:"Streamline Menu", sub:"Fewer SKUs, deeper pantry, faster kitchen", color:C.blue,
    preview:["Reduces delivery dependency","Fried foods lead in sports environments","Local rules everywhere else"],
  ...
  <div className={`pillar-card ${card.color==='#FF6700'?"card-orange":"card-blue"} ${isActive?"active":""}`} onClick={onToggle}>
    <div style={{ padding:"22px 20px 20px 20px", display:"flex", flexDirection:"column", height:"100%" }}>
      ...
      <div style={{ marginTop:"auto", paddingTop:16, display:"flex", justifyContent:"flex-end" }}>
        <div style={{ display:"inline-flex", alignItems:"center", gap:5, fontSize:14, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", color:"#fff", background:C.orange, padding:"5px 12px", borderRadius:20 }}>
          EXPAND <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><polygon points="2,1 9,5 2,9" fill="currentColor"/></svg>
        </div>
      </div>
    </div>
  </div>
  ...
  <Sec id="where" num="02" label="Travel Patterns" accent={C.orange}>
    ...
    {activeSupply && (() => { const c=supplyCards.find(x=>x.id===activeSupply); return c ? (
      <CardModal onClose={()=>setActiveSupply(null)} color={c.color} title={c.label} sub={c.sub}>
        ...
        {c.id==="rep" && <div style={{ marginTop:24, paddingTop:20, borderTop:`1px solid ${C.border}` }}><GenButton generatorId="sales_rep" label="Generate a Distributor Conversation Prompt"/></div>}
        {c.id==="stockpile" && <div style={{ marginTop:24, paddingTop:20, borderTop:`1px solid ${C.border}` }}><GenButton generatorId="stockpile" label="Generate a Stockpile Planning Prompt"/></div>}
      </CardModal>
    ) : null; })()}

    <div className="card-blue" style={{ background: C.surface, borderRadius: 12, marginTop:24, marginBottom: 24, overflow:"hidden" }}>
      <div style={{ padding:"24px 28px 0" }}>
        <div className="d" style={{ fontSize: 22, fontWeight: 800, color: C.text, marginBottom: 22, lineHeight: 1.15, textTransform:"uppercase", letterSpacing:".04em" }}>The Dominos<span style={{ color: C.dim, fontWeight: 400, fontSize: 16, textTransform:"none", letterSpacing:"normal" }}>: Brazil 2014, Russia 2018, Qatar 2022</span></div>
        <div style={{ position:"relative" }}>
          {[
            ...
            <div key={i} className="chain-node" style={{ paddingLeft:40, display: item.hidden && !chainOpen ? "none" : "block" }}>
              ...
              <p style={{ fontSize:16, color:item.bold?C.text:C.muted, lineHeight:1.75, fontWeight:item.bold?700:400 }}>{item.text}</p>
            </div>
          ))}
          {!chainOpen && (
            <div style={{ position:"absolute", bottom:0, left:0, right:0, height:"60%", background:`linear-gradient(to bottom, transparent, ${C.surface})`, pointerEvents:"none" }} />
          )}
        </div>
      </div>
      ...
      <button onClick={() => setChainOpen(o => !o)} style={{ width:"100%", background:"none", border:"none", borderTop:`1px solid ${C.border}`, padding:"16px 28px", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:16 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", transition:"transform .3s", transform: chainOpen ? "rotate(90deg)" : "none" }}>
          <svg width="14" height="14" viewBox="0 0 10 10" fill="none"><polygon points="2,1 9,5 2,9" fill="currentColor"/></svg>
        </div>
      </button>
    </div>
    ...
    <div className="callout-left-orange" style={{ display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ marginTop:"auto", display:"flex", justifyContent:"flex-end" }}>
        <button onClick={() => document.getElementById("pattern")?.scrollIntoView({ behavior:"smooth" })} style={{ background:C.orange, border:"none", cursor:"pointer", color:"#fff", fontSize:14, fontWeight:700, fontFamily:"'Jost',sans-serif", letterSpacing:".1em", textTransform:"uppercase", display:"inline-flex", alignItems:"center", gap:5, padding:"5px 12px", borderRadius:20 }}>
          THE PATTERN <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><polygon points="2,1 9,5 2,9" fill="currentColor"/></svg>
        </button>
      </div>
    </div>
...
const positionCards = [
  { id:"walkable", label:"Walkable to Venue", sub:"Inside the highest-intensity demand window", color:C.blue,
    preview:["36% post-match F&B lift applies to you","Post-match social window: 2-4 hours","Velocity is your lead decision"],
  ...
  { id:"secondary", label:"Secondary Market", sub:"30-150 miles out (the Saransk position)", color:C.blue,
    preview:["Saransk 2018: 18x foreign spending increase","Non-match day sustained demand is your window","Visibility is your lead pillar"],
...
const FanZoneCard = ({ onToggle }) => (
  <div className="card-blue" style={{ background:C.surface, borderRadius:12, cursor:"pointer", marginTop:24, padding:"20px 22px", display:"flex", flexDirection:"column" }} onClick={onToggle}>
    ...
    <p style={{ fontSize:16, color:C.muted, lineHeight:1.75, marginBottom:16 }}>
    ...
    <div style={{ marginTop:"auto", display:"flex", justifyContent:"flex-end" }}>
      <div style={{ display:"inline-flex", alignItems:"center", gap:5, fontSize:14, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", color:"#fff", background:C.blue, padding:"5px 12px", borderRadius:20 }}>
        EXPAND <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><polygon points="2,1 9,5 2,9" fill="currentColor"/></svg>
      </div>
    </div>
  </div>
...
const SPosition = () => {
  ...
  <Sec id="position" num="03" label="Your Position" accent={C.blue}>
    ...
    <div className="callout-left-orange" style={{ marginTop:10, cursor:"pointer", display:"flex", flexDirection:"column" }} onClick={() => { setSfStaffOpen(o => !o); setFzOpen(false); setActive(null); }}>
      ...
      <p style={{ fontSize:16, color:C.muted, lineHeight:1.75, marginBottom:16 }}>Fan zone operators are hiring your best bartenders at $40-60/hour plus tips. They will see these postings before June.</p>
      <div style={{ marginTop:"auto", display:"flex", justifyContent:"flex-end" }}>
        <div style={{ display:"inline-flex", alignItems:"center", gap:5, fontSize:14, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", color:"#fff", background:C.orange, padding:"5px 12px", borderRadius:20 }}>
          EXPAND <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><polygon points="2,1 9,5 2,9" fill="currentColor"/></svg>
        </div>
      ...
      <CardModal onClose={()=>setActive(null)} color={c.color} title={c.label} sub={c.sub}>
        <div style={{ display:"flex", gap:24, flexWrap:"wrap", alignItems:"flex-start" }}>
          <div style={{ flex:1, minWidth:240 }}>
            ...
            {c.note && <p style={{ fontSize:16, color:C.dim, lineHeight:1.78, borderTop:`1px solid ${C.border}`, paddingTop:14, fontStyle:"italic" }}>{c.note}</p>}
          </div>
      ...
      <CardModal onClose={()=>setFzOpen(false)} color={C.blue} title="Proximity" sub="39 days of sustained daily demand">
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          <div style={{ paddingTop:20, borderTop:`1px solid ${C.border}`, display:"flex", gap:10, flexWrap:"wrap" }}>
            <GenButton generatorId="gbp" label="Google Business Profile Prompt"/>
            <GenButton generatorId="compliance" label="Compliance Research Prompt"/>
          </div>
        </div>
      </CardModal>
    )}
    ...
    {sfStaffOpen && (
      <CardModal onClose={()=>setSfStaffOpen(false)} color={C.orange} title="Fan Zone Staffing Competition" sub="Fan zone operators are hiring from your labor pool">
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          <div style={{ paddingTop:20, borderTop:`1px solid ${C.border}`, display:"flex", gap:10, flexWrap:"wrap" }}>
            <GenButton generatorId="labor" label="Labor Planning Prompt"/>
            <GenButton generatorId="staff_briefing" label="Staff Briefing Prompt"/>
            <GenButton generatorId="surge_hiring" label="Surge Hiring Prompt"/>
            <GenButton generatorId="tip_structure" label="Tip Structure Prompt"/>
          </div>
        </div>
      </CardModal>
    )}
...
const Step = ({ num, children, color }) => (
  <div style={{ display:"flex", gap:14, marginBottom:18 }}>
    ...
    <p style={{ fontSize:16, color:C.muted, lineHeight:1.75 }}>{children}</p>
  </div>
);
...
// ─── COMPLETE GENERATORS REPLACEMENT ────────────────────────────────────────
// Drop this in place of the existing generators object in App.jsx
// All 11 generators rebuilt with XML structure, dynamic branching, platform reference
...
const generators = {
  ...
  stockpile: {
    ...
    build: (a) => {
      ...
      const task = tight || unknown
        ? `Once you have what you need: a prioritized stockpile plan that accounts for real physical constraints and protects the highest-risk category first. Physical space is not an afterthought - it is the plan.`
        : `Once you have what you need: a prioritized stockpile plan with a realistic timeline that gets the right inventory in before the surge arrives.`;
      ...
      return [
        ...
        "",
        "<reference>",
        "The research behind this conversation is documented at infiniterestaurants.ai - specifically the supply chain and travel patterns sections. If you have web access, read those sections before advising. The confidence tiers matter: some figures are primary confirmed, some are planning anchors. Knowing which is which sharpens the advice.",
        "</reference>",
        "",
        "<task>",
        "Bring everything you already know about this operator and their business into this conversation. Ask what you don't know. Don't generalize where you can be specific.",
        "",
        task,
        "</task>",
      ].join("\n");
  ...
  sales_rep: {
    ...
    build: (a) => {
      const frequent = a.frequency?.toLowerCase().includes("daily") ||
                       a.frequency?.toLowerCase().includes("3x");
      ...
      const task = strong
        ? `Once you have what you need: help me prepare for a specific planning conversation with someone who already knows my operation. The goal is to get ahead of supply chain pressure before it becomes a problem, not to sound alarmist about something my rep may already be thinking about.`
        : newRep
        ? `Once you have what you need: help me prepare to introduce this supply chain concern to someone I don't have history with yet. The goal is to open a productive planning relationship, not to make a list of demands.`
        : `Once you have what you need: help me shift an existing transactional relationship into a planning conversation. What to ask for, what to listen for, how to frame it so it lands as a partnership, not a complaint.`;
      ...
      return [
        ...
        "",
        "<operator>",
        `Delivery frequency is ${a.frequency?.toLowerCase()}. Rep relationship: ${a.relationship?.toLowerCase()}. ${infrequent ? "Already receiving infrequent deliveries means less buffer if the window compresses." : "Current frequency gives some working room, but that room shrinks under surge conditions."}`,
        "</operator>",
        "",
        "<reference>",
        "The supply chain research behind this conversation is documented at infiniterestaurants.ai - specifically the travel patterns and supply chain sections. If you have web access, read those before advising. They document what happened to delivery corridors in Brazil 2014, Russia 2018, and Qatar 2022.",
        "</reference>",
        "",
        "<task>",
        "Bring everything you already know about this operator and their business into this conversation. Ask what you don't know - their market, their critical items, their storage situation, their history with this rep.",
        "",
        task,
        "</task>",
      ].join("\n");
  ...
  labor: {
    ...
    build: (a) => {
      ...
      const notNear = a.proximity?.toLowerCase().includes("not near");

      const laborContext = nearFanZone
        ? `Fan zone operators and event activations are recruiting hospitality staff at $40-60 per hour plus tips for the full 39-day window. That is rates most independent operators cannot match for a month-long stretch. Bartenders, beertenders, and FOH captains will see these postings on Poached and Indeed before June. The operator is in direct competition with that labor market.`
        : secondary
      ...
      return [
        ...
        "",
        "<operator>",
        `Proximity to fan zone or host city: ${a.proximity}. Biggest labor concern: ${a.concern}.`,
        nearFanZone
          ? `Direct competition with fan zone labor market makes this a two-front problem: keep the staff already here, and find surge capacity on top of that.`
          : `The labor challenge is internal - managing through the demand pattern, not competing with event-operator wages.`,
        "</operator>",
        "",
        "<reference>",
        "The labor and operations research behind this conversation is documented at infiniterestaurants.ai - specifically the prep priorities section. If you have web access, read the labor pillar before advising.",
        "</reference>",
        "",
        "<task>",
        "Bring everything you already know about this operator, their team, their operation, into this conversation. Ask what you don't know - team size, key roles, current compensation, scheduling patterns, what's worked before.",
        "",
        "Once you have what you need: a labor strategy that actually fits this operation. Not a generic checklist - a plan shaped by who they are and what they're walking into.",
        "</task>",
      ].join("\n");
    }
  },
  ...
  gbp: {
    ...
    build: (a) => {
      const current = a.status?.toLowerCase().includes("up to date");
      ...
      const mixed = a.visitors?.toLowerCase().includes("mixed");
      const domestic = a.visitors?.toLowerCase().includes("domestic");
      ...
      const visitorContext = latinAmerican
        ...
        : `The visitor mix is mostly domestic - but the Super Tourist profile still applies to any international visitors who are here, and domestic fans also search on their phones before choosing where to go.`;
      ...
      return [
        ...
        "",
        "<context>",
        "Super Tourists do not rely on local knowledge. They have phones. They search near their hotel, near the fan zone, near wherever they are standing. Google decides who shows up in that search - not the sign on the door, not Yelp, not word of mouth from a local. A restaurant that isn't optimized for that search doesn't exist to the visitor most likely to spend $160 on dinner and stay for three hours.",
        "",
        visitorContext,
        "",
        statusContext,
        ...
        "",
        "<operator>",
        `Current GBP status: ${a.status}. Most likely international visitor markets: ${a.visitors}.`,
        "</operator>",
        "",
        "<reference>",
        "The digital discovery research behind this conversation is documented at infiniterestaurants.ai - specifically the prep priorities and Super Tourist sections. If you have web access, read those before advising. The 'football not soccer' search behavior is documented there.",
        "</reference>",
        "",
        "<task>",
        "Bring everything you already know about this operator, their concept, their location, their hours into this conversation. Ask what you don't know - what makes this place worth finding, what the viewing setup looks like, what languages might be relevant.",
        "",
        none
          ? "Once you have what you need: start from zero and build a GBP that will show up for the right searches before June 11."
          : "Once you have what you need: a specific optimization plan - what to update, what attributes to enable, what description to write - that gets this restaurant found by international visitors searching on a phone.",
        "</task>",
      ].join("\n");
    }
  },
  ...
  compliance: {
    ...
    build: (a) => {
      ...
      return [
        ...
        licenseContext,
        "",
        "The research behind this compliance context is documented at infiniterestaurants.ai - specifically the patterns and sources section which covers broadcast licensing frameworks, safe harbor marketing terminology, and regulatory patterns from prior tournaments. If you have web access, read that section alongside your jurisdiction research.",
        "</context>",
        "",
        "<task>",
        "Research each of the following areas. For every item return: (1) whether it applies to this operation type and address, (2) what the actual requirement is, (3) the deadline or lead time, (4) a verification line: who to contact to confirm, agency name, website, phone number if findable, and exactly what to ask for.",
        "",
        "Research areas:",
        "1. Commercial broadcast licensing for showing World Cup matches. What triggers a FIFA Public Viewing License for a commercial establishment. What are the safe harbor conditions. Who issues licenses and how.",
        ...
        "4. Temporary outdoor seating or infrastructure permits. If adding a patio, service window, or temporary structure: what permits are required in this city, and are there any construction freeze dates or permit moratoriums during the tournament window.",
        ...
        "",
        "Return as a checklist: each item marked APPLIES / DOES NOT APPLY / NEEDS VERIFICATION. End every item with: 'To verify: [specific agency or contact, website, what to ask].' Flag anything with a deadline before June 11 at the top of the list.",
        "</task>",
      ].join("\n");
    }
  },
  ...
  social: {
    ...
    build: (a) => {
      ...
      const both = a.platforms?.toLowerCase().includes("instagram + facebook") ||
                   a.platforms?.toLowerCase().includes("all");
      const tiktok = a.platforms?.toLowerCase().includes("tiktok");
      const viewing = a.angle?.toLowerCase().includes("showing matches");
      const dining = a.angle?.toLowerCase().includes("dining experience");
      ...
      const angleContext = viewing
        ? `The viewing angle requires specific, credible content: actual kickoff times, which matches are being screened, what the viewing setup looks like. International visitors are comparing options on their phones. Specificity is what builds trust. "Come watch the big game" is noise. "Argentina vs France, 2 PM, full audio, $5 pints until kickoff" is information.`
        : dining
      ...
      return [
        ...
        angleContext,
        "",
        "One hard constraint: there are trademark restrictions on FIFA marks, logos, and the official tournament name. 'International football,' 'summer soccer,' or 'the 2026 tournament' are safe. The official emblem and trophy image are not. International visitors also commonly search using 'football' not 'soccer' - one word in content and profile descriptions changes who finds this restaurant.",
        "</context>",
        "",
        "<operator>",
        `Active platforms: ${a.platforms}. Primary angle: ${a.angle?.toLowerCase()}.`,
        "</operator>",
        "",
        "<reference>",
        "The discovery and marketing research behind this conversation is documented at infiniterestaurants.ai - specifically the prep priorities section covering digital discovery and safe marketing. If you have web access, read that section before advising. It covers the trademark safe harbor framework and the 'football not soccer' search behavior.",
        "</reference>",
        "",
        "<task>",
        "Bring everything you already know about this operator, their concept, their voice, their audience into this conversation. Ask what you don't know - posting frequency, tone, specific events to promote, content already created.",
        "",
        "Once you have what you need: a content plan that reaches both regulars and international visitors, uses safe marketing language, and doesn't sound like every other restaurant doing generic tournament content.",
        "</task>",
      ].join("\n");
    }
  },
  ...
  menu_fast_track: {
    ...
    build: (a) => {
      ...
      const strongLocal = a.challenge?.toLowerCase().includes("central");
      const someLocal = a.local?.toLowerCase().includes("some local");
      ...
      const localContext = strongLocal || someLocal
        ...
        : `Without a strong local identity, the Fast Track menu needs to anchor on what this kitchen does best at speed - the items with the highest quality-to-ticket-time ratio, regardless of origin.`;
      ...
      return [
        ...
        "",
        "<operator>",
        `What breaks first under pressure: ${a.challenge}. Local product identity: ${a.local}.`,
        "</operator>",
        "",
        "<reference>",
        "The menu strategy research behind this conversation is documented at infiniterestaurants.ai - specifically the prep priorities section covering Fast Track menus, local product strategy, and the Locals menu concept. If you have web access, read that section before advising.",
        "</reference>",
        "",
        "<task>",
        "Bring everything you already know about this kitchen and concept into this conversation. Ask what you don't know - current menu size, highest-margin items, what causes the most friction, what the operator is most proud of.",
        "",
        "Once you have what you need: a Fast Track configuration that holds quality under volume, leads with what makes this place different from anywhere else, and a parallel Locals strategy that keeps regulars feeling like they belong here through the summer.",
        "</task>",
      ].join("\n");
    }
  },
  ...
  tip_structure: {
    ...
    build: (a) => {
      ...
      const fohPool = a.current?.toLowerCase().includes("foh") &&
                      !a.current?.toLowerCase().includes("boh");
      const fullPool = a.current?.toLowerCase().includes("foh and boh");
      const tipOut = a.current?.toLowerCase().includes("tip out");
      ...
      const heavyBOH = a.change?.toLowerCase().includes("boh workload");
      ...
      const structureContext = serversOnly
        ...
        : `Current structure exists but may not hold when the conditions change. Extended hours, heavier BOH loads, and international guests who tip differently than domestic regulars all put pressure on whatever the current model is.`;

      const pressureContext = allChanges
        ? `All three pressure points are in play simultaneously: extended hours change the earning window, heavier BOH load changes who's working hardest, and international guests who tip differently change the total pool. The current structure is being asked to handle a fundamentally different operating environment.`
        : guestTipping
        ? `International visitors tip differently than domestic regulars - some cultures tip generously by US standards, some don't tip at all, and contactless prompts don't always translate across cultural expectations. The total pool may be different from what the team is used to, for better or worse.`
        : extendedHours
      ...
      return [
        ...
        "",
        "<operator>",
        `Current tip structure: ${a.current}. What's changing this summer: ${a.change}.`,
        "</operator>",
        "",
        "<reference>",
        "The labor and retention research behind this conversation is documented at infiniterestaurants.ai - specifically the prep priorities section covering tip structure, extended hours, and BOH retention. If you have web access, read that section before advising.",
        "</reference>",
        "",
        "<task>",
        "Bring everything you already know about this operator and their team into this conversation. Ask what you don't know - team size, FOH/BOH split, current compensation levels, what the operator is most worried about.",
        "",
        "Once you have what you need: a tip and compensation approach that keeps the whole team invested and showing up through July 19, without creating resentment or legal exposure.",
        "</task>",
      ].join("\n");
    }
  },
  ...
  staff_briefing: {
    ...
    build: (a) => {
      ...
      const mixed = a.awareness?.toLowerCase().includes("mixed");
      ...
      const lowKey = a.tone?.toLowerCase().includes("low key");
      ...
      const toneContext = opportunity
        ...
        : `Low-key framing. We've handled busy before. The goal is to calibrate expectations without creating drama - acknowledge what's different, explain the preparation, and let the team draw their own conclusions about the scale.`;
      ...
      return [
        ...
        "",
        "<operator>",
        `Team's current awareness level: ${a.awareness}. Tone to set: ${a.tone}.`,
        "</operator>",
        "",
        "<reference>",
        "The operational context behind this briefing is documented at infiniterestaurants.ai. If you have web access, read the Super Tourist section and the prep priorities section before advising - the specifics about who's coming, how they behave, and what the operation needs to do differently are what make a briefing credible instead of generic.",
        "</reference>",
        "",
        "<task>",
        "Bring everything you already know about this operator, their team, and their operation into this conversation. Ask what you don't know - how many people, what roles, what they're most concerned about communicating, what's already been decided about hours and prep.",
        "",
        "Once you have what you need: a staff briefing written in plain language, deliverable in person, that prepares the team for what's actually coming without sounding like a corporate memo.",
        "</task>",
      ].join("\n");
    }
  },
  ...
  surge_hiring: {
    ...
    build: (a) => {
      ...
      const support = a.roles?.toLowerCase().includes("support");
      const june11 = a.timeline?.toLowerCase().includes("june 11");
      const midMay = a.timeline?.toLowerCase().includes("mid-may");
      const endMay = a.timeline?.toLowerCase().includes("end of may");
      const behind = a.timeline?.toLowerCase().includes("already behind");

      const rolesContext = foh
        ? `FOH roles - servers and bartenders - are the most competitive to fill right now. Fan zone operators started posting at $40-60/hr plus tips months ago. The people worth hiring have options. Lead time and what the role actually offers - not just the wage - is what separates a successful hire from a ghost.`
        : boh
        ? `BOH roles are harder to fill in any summer hiring market. The surge adds pressure without adding glamour. What works: reaching culinary students available for summer, former employees worth calling back, and being specific about what the work actually involves rather than posting a generic line cook ad.`
        : bothRoles
      ...
      const timelineContext = behind
        ? `Already behind. The operators who started in February have a trained bench. The operators posting now are competing for a smaller pool of available people. The hiring plan needs to be realistic about what's actually possible in the time remaining and prioritize ruthlessly.`
        : june11
        ? `Targeting tournament open with no buffer for training. That is a tight window. Anyone hired at the end of May needs to be operational within days, not weeks. The hiring criteria need to account for that - experienced people who can onboard fast.`
        : midMay
        ? `Mid-May target gives legitimate training time. That is the right approach. Post now, interview through April, onboard in early May, train through the month.`
        : `End of May is tight but workable if hiring starts immediately. Two weeks of training before the tournament opens is the minimum viable window for a new hire to be useful rather than a liability during peak demand.`;
      ...
      return [
        ...
        "",
        "<context>",
        "Fan zone operators, official activations, and event hospitality companies started recruiting for the same labor pool months ago. Every week of delay is a week the best available candidates are being interviewed elsewhere. The operators who built their surge bench early have it. The ones starting now are working with what's left - which is still workable, but requires a different approach.",
        "",
        rolesContext,
        "",
        timelineContext,
        ...
        "",
        "<operator>",
        `Roles needed: ${a.roles}. Target ready date: ${a.timeline}.`,
        "</operator>",
        "",
        "<reference>",
        "The labor market context behind this conversation is documented at infiniterestaurants.ai - specifically the prep priorities section covering surge hiring and retention. If you have web access, read that section before advising.",
        "</reference>",
        "",
        "<task>",
        "Bring everything you already know about this operator, their market, and their operation into this conversation. Ask what you don't know - what they can offer beyond wage, what's worked in past hiring, what the operation actually asks of people in these roles.",
        "",
        behind
          ? "Once you have what you need: a realistic hiring plan for the time that actually remains. Not what should have been done in February - what can be done now."
          : "Once you have what you need: a hiring plan with a timeline, sourcing approach, and onboarding sequence that gets the right people trained and operational before the surge arrives.",
        "</task>",
      ].join("\n");
    }
  },
  ...
  pricing_bundles: {
    ...
    build: (a) => {
      const matchDay = a.daypart?.toLowerCase().includes("match-day");
      ...
      const leavingMoney = a.risk?.toLowerCase().includes("leaving money");
      ...
      const daypartContext = matchDay
        ? `Match-day viewing windows are the highest-intensity revenue opportunity. A Super Tourist spending $140-180 a day on food isn't hunting for a deal. They're looking for a good experience. Bundle pricing - drink plus appetizer, watch-party package, pre-match set menu - drives higher average tickets without requiring individual price changes.`
        : lateNight
      ...
      const riskContext = leavingMoney
        ? `Leaving money on the table is the real risk here. Super Tourists are not price-sensitive in the way regular diners are. They are spending $400+ a day total. Food is a pleasure, not a budget line. The question is what bundle structure captures that spending without feeling extractive.`
        : pricingOut
        ...
        : bundleDesign
        ? `Bundle design is where most operators underdeliver. A bundle that bundles the wrong things, at the wrong price point, for the wrong daypart doesn't drive higher tickets - it creates confusion. The design question is: what does the guest actually want to combine, and what margin does that combination produce.`
      ...
      return [
        ...
        "",
        "<context>",
        "The Super Tourist profile is specific: $140-180 per person per day on food and beverage, groups of 4-8, table dwell times of 90-120 minutes, and no particular price sensitivity. They are not hunting for a deal. They are looking for a good experience in a place that feels worth visiting. Pricing and bundle strategy that reflects that profile captures significantly more revenue than holding flat rates through the tournament window.",
        "",
        daypartContext,
        "",
        riskContext,
        ...
        "",
        "<operator>",
        `Most focused service periods: ${a.daypart}. Biggest pricing concern: ${a.risk}.`,
        "</operator>",
        "",
        "<reference>",
        "The Super Tourist spending profile and pricing strategy research behind this conversation is documented at infiniterestaurants.ai - specifically the Super Tourist section and the prep priorities operations pillar. If you have web access, read those sections before advising. The $140-180 daily F&B spend figure is a planning anchor derived from Russia 2018 and Brazil 2014 data.",
        "</reference>",
        "",
        "<task>",
        "Bring everything you already know about this operator, their concept, their price point, their guest mix into this conversation. Ask what you don't know - current average ticket, highest-margin items, what has and hasn't worked in past promotions.",
        "",
        "Once you have what you need: a bundle strategy and pricing approach that fits this specific operation - higher average tickets through smart design, a local retention strategy that keeps regulars feeling like they belong here.",
        "</task>",
      ].join("\n");
    }
  },
  ...
  tech_readiness: {
    ...
    questions: [
      { id:"concern", text:"Where does your tech feel most exposed heading into a high-demand stretch?",
        options:["Payment: contactless, terminals, tip flow",
                 ...
                 "POS: configuration, menu switching, reporting",
                 "Reservations and waitlist: capacity, dwell times, covers",
                 "I don't know what I don't know: give me the full audit"] },
      { id:"platforms", text:"Which platforms are you currently running?",
        options:["POS only: no delivery or online ordering",
                 "POS plus one delivery app",
                 "POS plus multiple delivery and/or online ordering apps",
                 "Reservation system plus POS",
                 "Full stack: POS, delivery, reservations, online ordering",
    ...
    build: (a) => {
      ...
      const menus = a.concern?.toLowerCase().includes("menus");
      const pos = a.concern?.toLowerCase().includes("pos");
      const reservations = a.concern?.toLowerCase().includes("reservations");
      const fullAudit = a.concern?.toLowerCase().includes("don't know");
      const fullStack = a.platforms?.toLowerCase().includes("full stack");
      const posOnly = a.platforms?.toLowerCase().includes("pos only");
      const notSure = a.platforms?.toLowerCase().includes("not sure");

      const concernContext = fullAudit
        ? `Full audit requested. Every tech surface is a potential failure point during sustained high-volume demand: payment friction slows the highest-revenue tables, uncapped delivery queues compound kitchen tickets, outdated digital menus create friction before the guest orders, misconfigured POS misses the data needed to manage the window in real time.`
        : payment
        ? `Payment infrastructure is the right place to focus. Contactless payments accounted for 45% of all host-city purchases in Russia 2018 - that was 2018. International visitors arriving from Europe, Latin America, and Australia expect tap-to-pay as a baseline. A declined Apple Pay at the highest-spending table in the dining room is friction at the worst possible moment. Contactless processing is also approximately 10x faster per transaction than card swipe - at post-match volume, that speed difference determines whether a queue converts or walks.`
        : online
        ? `Online and off-premise order management is the right place to focus. Every delivery platform has a pause button. Most operators don't know exactly where it is until they need it - which is always during peak service when there's no time to figure it out. An uncapped delivery queue printing into a kitchen running at capacity compounds every seated ticket.`
        : menus
        ? `Digital menus are the right place to focus. A menu that doesn't load fast on mobile, isn't accurate across platforms, or can't be switched to a high-volume configuration without a manual update on six platforms is a liability during the tournament window.`
        : pos
        ? `POS configuration is the right place to focus. Extended hours, bundle pricing, Fast Track menu variants, and daypart reporting all require POS setup that most operators haven't done. The tournament window is not the time to discover those configurations don't exist.`
        : `Reservations and waitlist configuration is the right place to focus. A system configured for 60-minute table turns misquotes wait times when tables are staying 90-120 minutes. That mismatch creates disappointment before the guest sits down.`;

      const platformContext = fullStack
        ? `Full stack operation means more surfaces to audit and more potential failure points, but also more control. Every platform needs to be configured specifically for the tournament window before June 11.`
        : posOnly
        ? `POS only - no delivery or online ordering complexity. The audit is more contained: payment infrastructure, menu configuration, and reporting setup.`
        : notSure
        ? `Unclear what platforms are in the stack. That ambiguity needs to be resolved before any meaningful tech audit is possible - you can't configure what you can't identify.`
        : `Multiple platforms in the stack. Each one has volume controls, configuration options, and failure modes that need to be addressed before the surge arrives.`;

      return [
        "<role>",
        "You are a restaurant technology advisor conducting a pre-surge tech readiness audit for an independent operator who needs to find and fix every potential failure point before June 11.",
        "</role>",
        "",
        "<context>",
        "Tech failures during a sustained high-demand period are not recoverable in real time. A payment terminal that rejects contactless, a delivery channel that can't be paused, a digital menu that doesn't load on mobile, a POS that can't switch to a fast-track configuration mid-service - each one creates friction at the moments of highest revenue potential. The audit happens now, not during the tournament.",
        "",
        concernContext,
        "",
        platformContext,
        ...
        "",
        "<operator>",
        `Biggest area of tech concern: ${a.concern}. Current platform situation: ${a.platforms}.`,
        "</operator>",
        "",
        "<reference>",
        "The tech readiness research behind this conversation is documented at infiniterestaurants.ai - specifically the prep priorities tech pillar and the contactless payment data from Russia 2018 and Qatar 2022. If you have web access, read those sections before advising.",
        "</reference>",
        "",
        "<task>",
        "Bring everything you already know about this operator and their tech stack into this conversation. Ask what you don't know - specific platforms and versions, current configuration, what has failed before.",
        "",
        fullAudit
          ? "Once you have what you need: a full tech audit covering all five areas - payment, online orders, digital menus, POS, and reservations - prioritized by failure risk and with specific actions for each."
          : `Once you have what you need: a focused audit of the ${a.concern?.toLowerCase()} area - what to check, what to configure, what to test before June 11. Specific and executable, not a generic tech checklist.`,
        "</task>",
      ].join("\n");
    }
  },
      ...
      <div onClick={e => e.stopPropagation()} style={{ background:C.surface, border:`2px solid ${accentColor}`, borderRadius:14, width:"100%", maxWidth:680, maxHeight:"85vh", overflow:"auto", boxShadow:`0 24px 64px rgba(0,0,0,.6), 0 0 40px ${gen.color==="orange"?"rgba(255,103,0,.15)":"rgba(0,103,255,.15)"}` }}>
        ...
        <div style={{ padding:"20px 24px", borderBottom:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, background:C.surface, zIndex:1 }}>
          <div>
            ...
            <div className="d" style={{ fontSize:19, fontWeight:800, color:C.text }}>{gen.label}</div>
          </div>
          ...
          {!prompt ? (
            <>
              ...
              <div style={{ display:"flex", gap:6, marginBottom:28 }}>
                {questions.map((q,i) => (
                  <div key={i} style={{ height:3, flex:1, borderRadius:2, background: i < step ? accentColor : i === step ? accentColor : C.border, opacity: i < step ? 1 : i === step ? 1 : 0.4, transition:"background .2s" }}/>
                ))}
              </div>

              {/* Current question */}
              <div style={{ marginBottom:20 }}>
                <div className="d" style={{ fontSize:19, fontWeight:700, color:C.text, lineHeight:1.3, marginBottom:20 }}>{currentQ.text}</div>
                {currentQ.type === "text" ? (
                  <div>
                    <input
                      type="text"
                      ...
                      onKeyDown={e => { if(e.key==="Enter" && answers[currentQ.id]?.trim()) { if(step < questions.length-1) { setStep(s=>s+1); } else { setPrompt(gen.build({...answers})); } } }}
                    />
                    <button
                      onClick={() => { if(answers[currentQ.id]?.trim()) { if(step < questions.length-1) { setStep(s=>s+1); } else { setPrompt(gen.build({...answers})); } } }}
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                  {currentQ.options.map((opt,i) => (
                    <button key={i} onClick={() => handleAnswer(currentQ.id, opt)}
                      ...
                      {opt}
                      {answers[currentQ.id]===opt && <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M2 7l4 4 6-6" stroke={accentColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                    </button>
                  ))}
                </div>
                )}
            ...
            <>
              ...
              <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
                <button onClick={handleCopy} style={{ background: copied ? "rgba(0,103,255,.15)" : accentColor, border:"none", borderRadius:8, padding:"12px 24px", cursor:"pointer", fontSize:14, fontWeight:700, color: copied ? C.blue : "white", fontFamily:"'Jost',sans-serif", letterSpacing:".06em", textTransform:"uppercase", transition:"all .2s", display:"flex", alignItems:"center", gap:8 }}>
                  {copied ? <>Copied <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M2 7l4 4 6-6" stroke={C.blue} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg></> : "Copy Prompt"}
                </button>
...
// Button component to trigger a generator
  ...
  return (
    <>
      <button onClick={e => { e.stopPropagation(); setOpen(true); }}
        ...
        onMouseLeave={e => e.currentTarget.style.background="none"}>
        <svg width="11" height="11" viewBox="0 0 11 11" fill="none"><rect x="1" y="1" width="3.5" height="3.5" rx="0.5" fill={color}/><rect x="6.5" y="1" width="3.5" height="3.5" rx="0.5" fill={color}/><rect x="1" y="6.5" width="3.5" height="3.5" rx="0.5" fill={color}/><rect x="6.5" y="6.5" width="3.5" height="3.5" rx="0.5" fill={color}/></svg>
...
const AIPromptsCell = () => {
  ...
  return (
    <div style={{ marginTop:16 }}>
      <div className="card-blue" style={{ background:C.surface, borderRadius:12, overflow:"hidden" }}>
        <button onClick={() => setOpen(o => !o)} style={{ width:"100%", background:"none", border:"none", cursor:"pointer", textAlign:"left", padding:"22px 24px 22px", display:"flex", flexDirection:"column" }}>
          <div>
            <div style={{ fontSize:18, fontWeight:700, letterSpacing:".12em", textTransform:"uppercase", color:C.text, marginBottom:6 }}>AI Prompts:</div>
            <div style={{ fontSize:18, fontWeight:400, letterSpacing:".08em", textTransform:"uppercase", color:C.text, lineHeight:1.2, marginBottom:6, whiteSpace:"nowrap" }}>All the prompt generators in one place.</div>
            <p style={{ fontSize:14, color:C.dim, lineHeight:1.6, marginTop:6, maxWidth:560 }}>Plus guidance on which AI platforms to use, what good output looks like, and how to stay in the lead when AI is in the loop.</p>
          </div>
          <div style={{ marginTop:"auto", paddingTop:16, display:"flex", justifyContent:"flex-end" }}>
            <div style={{ display:"inline-flex", alignItems:"center", gap:5, fontSize:14, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", color:"#fff", background:C.blue, padding:"5px 12px", borderRadius:20 }}>
              {open ? "Collapse" : "Open"}
              <svg width="10" height="10" viewBox="0 0 10 10" fill="none" style={{ transition:"transform .3s", transform: open?"rotate(90deg)":"none" }}><polygon points="2,1 9,5 2,9" fill="currentColor"/></svg>
            </div>
          </div>
        </button>

        <div className={`xbody ${open?"open":""}`}>
          <div style={{ padding:"0 24px 32px", display:"flex", flexDirection:"column", gap:36 }}>
            ...
            <div style={{ borderTop:`1px solid ${C.border}`, paddingTop:28 }}>
              <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16 }}>
                <div style={{ display:"flex", gap:4 }}>
                  {["HI","AI","HI"].map((t,i) => (
                    <div key={i} style={{ background: t==="AI" ? C.blue : C.surf2, border:`1px solid ${t==="AI"?C.blue:C.border}`, borderRadius:5, padding:"5px 10px" }}>
                      <span className="n" style={{ fontSize:14, fontWeight:700, color: t==="AI" ? "white" : C.dim }}>{t}</span>
                    </div>
                  ))}
                </div>
                <div style={{ fontSize:14, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", color:C.dim }}>Human Intelligence & Instinct / AI Amplification / Human Intelligence & Instinct</div>
              </div>
              <p style={{ fontSize:16, color:C.text, lineHeight:1.82, marginBottom:14 }}>
                You've been reading rooms longer than AI has existed. The <strong style={{ color:C.text }}>H</strong> is Human or Hospitality. The <strong style={{ color:C.text }}>I</strong> is Instinct or Intellect. Both apply. You bring all four to every shift. AI brings none of them.
              </p>
              <p style={{ fontSize:16, color:C.muted, lineHeight:1.82, marginBottom:14 }}>
                AI excels at pattern recognition. It does not think. It does not decide. It does not know your dining room, your regulars, your rep, or your neighborhood. What it can do is expand what you're able to see and act on, fast. That's the amplification layer. You open the conversation. You close it. The output is only as good as the judgment you bring to both ends.
              </p>
              <div className="callout-left-blue" style={{ marginBottom:0 }}>
                ...
                <p style={{ fontSize:16, color:C.text, lineHeight:1.75, fontWeight:600 }}>If something feels off or seems wrong, it probably is. Pull that thread. Verify it. Your instincts are not a liability in an AI workflow. They are the first gate and the last gate. Don't surrender them.</p>
              </div>
            </div>
                ...
                {aiPlatforms.map((p,i) => (
                  <div key={i} style={{ borderBottom: i<aiPlatforms.length-1?`1px solid ${C.border}`:"none" }}>
                    <button onClick={() => setActivePlatform(activePlatform===i?null:i)} style={{ width:"100%", background:"none", border:"none", cursor:"pointer", padding:"14px 18px", display:"flex", alignItems:"center", justifyContent:"space-between", textAlign:"left" }}>
                      ...
                      <div style={{ transition:"transform .3s", transform: activePlatform===i?"rotate(90deg)":"none" }}>
                        <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><polygon points="2,1 9,5 2,9" fill={C.dim}/></svg>
                      </div>
                    </button>
            ...
            {/* ── ALL GENERATORS ── */}
            <div>
              <div style={{ fontSize:14, fontWeight:700, letterSpacing:".14em", textTransform:"uppercase", color:C.text, marginBottom:6 }}>All Prompt Generators</div>
              <p style={{ fontSize:14, color:C.dim, lineHeight:1.6, marginBottom:16 }}>These prompts are pre-structured with World Cup context. Each asks a few questions, then hands you a prompt built from your answers. Take it to whatever AI platform fits the task.</p>
              <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                {allGenerators.map(g => (
                  <GenButton key={g.id} generatorId={g.id} label={g.label}/>
                ))}
              </div>
              <div style={{ marginTop:16, paddingTop:16, borderTop:`1px solid ${C.border}` }}>
                <p style={{ fontSize:14, color:C.dim, lineHeight:1.7 }}>Don't see what you need? Your own context plus any AI platform is the most powerful prompt you can build. The HI-AI-HI framework above shows you how.</p>
              </div>
            </div>
...
const SAction = () => {
  ...
  return (
    <Sec id="action" num="04" label="Prep Priorities" accent={C.orange}>
      <h2 className="d" style={{ fontSize:"clamp(28px,4.2vw,48px)", fontWeight:800, lineHeight:1.04, letterSpacing:"-.02em", marginBottom:20, color:C.text }}>
        <span style={{ color: C.text }}>Keep your </span><span style={{ color: C.orange }}>knees bent</span><span style={{ color: C.text }}>.</span><br /><span style={{ color: C.blue }}>Better beats</span><span style={{ color: C.text }}> perfect.</span>
      </h2>
      <p style={{ fontSize:19, color:C.muted, fontWeight:300, lineHeight:1.78, marginBottom:12, maxWidth: "90%" }}>
        Food, labor, operations, tech. You run these four things every shift. What's different is who's walking in, how they found you, and what happens to your supply chain while they're here. A million visitors moving through North America by car for a month puts pressure on all four at the same time. Your shelves, your staff, your hours, your payment terminal - none of it is built for this. The cards below are organized around where the pressure actually lands. Start with the one that keeps you up at night.
      </p>
      <p style={{ fontSize:14, color:C.dim, marginBottom:24, letterSpacing:".1em", textTransform:"uppercase", fontWeight:700 }}>
      ...
      {active && (() => { const p=pillars.find(x=>x.id===active); return p ? (
        <CardModal onClose={()=>setActive(null)} color={p.color} title={p.label} sub={p.sub}>
          ...
          <div style={{ marginTop:24, paddingTop:20, borderTop:`1px solid ${C.border}`, display:"flex", gap:10, flexWrap:"wrap" }}>
            {p.id==="food" && <><GenButton generatorId="stockpile" label="Stockpile Planning Prompt"/><GenButton generatorId="sales_rep" label="Distributor Conversation Prompt"/><GenButton generatorId="menu_fast_track" label="Fast Track Menu Prompt"/></>}
            {p.id==="labor" && <><GenButton generatorId="labor" label="Labor Planning Prompt"/><GenButton generatorId="staff_briefing" label="Staff Briefing Prompt"/><GenButton generatorId="surge_hiring" label="Surge Hiring Prompt"/><GenButton generatorId="tip_structure" label="Tip Structure Prompt"/></>}
            {p.id==="ops" && <><GenButton generatorId="gbp" label="Google Business Profile Prompt"/><GenButton generatorId="compliance" label="Compliance Research Prompt"/><GenButton generatorId="social" label="Social Media Prompt"/><GenButton generatorId="pricing_bundles" label="Pricing & Bundles Prompt"/><GenButton generatorId="tech_readiness" label="Tech Readiness Prompt"/></>}
            {p.id==="tech" && <><GenButton generatorId="tech_readiness" label="Tech Readiness Prompt"/><GenButton generatorId="compliance" label="Compliance Research Prompt"/><GenButton generatorId="gbp" label="Google Business Profile Prompt"/></>}
          </div>
        </CardModal>
      ) : null; })()}
      ...
      {sfOpen && (
        <CardModal onClose={()=>setSfOpen(false)} color={C.blue} title="Super Fan" sub="Become the Watch Venue, Get Found, Full Match Schedule">
          ...
          <div style={{ marginTop:24, paddingTop:20, borderTop:`1px solid ${C.border}`, display:"flex", gap:10, flexWrap:"wrap" }}>
            <GenButton generatorId="gbp" label="Google Business Profile Prompt"/>
            <GenButton generatorId="social" label="Social Media Content Prompt"/>
            <GenButton generatorId="compliance" label="Broadcast Licensing Prompt"/>
            <GenButton generatorId="labor" label="Labor Planning Prompt"/>
          </div>
        </CardModal>
      )}
        ...
        <div onClick={() => setOpen(false)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.78)", zIndex:600, display:"flex", alignItems:"center", justifyContent:"center", padding:24, backdropFilter:"blur(4px)", animation:"in .18s ease" }}>
          <div onClick={e => e.stopPropagation()} style={{ background:C.surface, border:`2px solid ${level===3?C.orange:C.blue}`, borderRadius:14, width:"100%", maxWidth:680, maxHeight:"80vh", overflow:"auto", boxShadow:"0 24px 64px rgba(0,0,0,.6)" }}>
            ...
            <div style={{ padding:"20px 24px" }}>
              <p style={{ fontSize:16, color:C.muted, lineHeight:1.78, marginBottom:20 }}>{modal.body}</p>
              {modal.figures && (
                <div style={{ marginBottom:20 }}>
              ...
              <div style={{ padding:"12px 24px", borderTop:`1px solid ${C.border}`, background:C.surf2, display:"flex", justifyContent:"flex-end", gap:8, flexWrap:"wrap" }}>
                {modal.link && (
                  <button onClick={() => { setOpen(false); document.getElementById(modal.link)?.scrollIntoView({behavior:"smooth"}); }}
                    style={{ fontSize:14, color:"#fff", fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", background:C.orange, border:"none", cursor:"pointer", display:"inline-flex", alignItems:"center", gap:5, padding:"5px 12px", borderRadius:20, fontFamily:"'Jost',sans-serif" }}>
  ...
  return (
    <div style={{ borderBottom:`1px solid ${C.border}` }}>
      <button onClick={onToggle} style={{ width:"100%", background:"none", border:"none", cursor:"pointer", padding:"16px 20px", display:"flex", alignItems:"flex-start", justifyContent:"space-between", textAlign:"left", gap:16 }}>
        ...
        <div style={{ transition:"transform .3s", transform: isOpen?"rotate(90deg)":"none", flexShrink:0, marginTop:2 }}>
          <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><polygon points="2,1 9,5 2,9" fill={C.dim}/></svg>
        </div>
      </button>
        ...
        <div style={{ padding:"0 20px 16px", display:"flex", flexDirection:"column" }}>
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:16 }}>
            ...
            {source.platform.map((p,i) => (
              <button key={i} onClick={() => document.getElementById(p)?.scrollIntoView({behavior:"smooth"})} style={{ background:"none", border:`1px solid ${C.border}`, borderRadius:4, padding:"3px 8px", cursor:"pointer", fontSize:14, fontWeight:700, letterSpacing:".08em", textTransform:"uppercase", color:C.muted, fontFamily:"'Jost',sans-serif" }}>
...
const SPattern = () => {
  ...
  const ExpandTrigger = ({ label, open, onToggle, accent = C.blue }) => (
    <button onClick={onToggle}
      ...
      <svg width="12" height="12" viewBox="0 0 10 10" fill="none" style={{ flexShrink:0, transition:"transform .3s", transform: open?"rotate(180deg)":"none" }}><polygon points="1,2 9,2 5,9" fill={accent}/></svg>
    </button>
  );
  ...
  <section id="pattern" style={{ borderTop:`1px solid ${C.border}`, padding:"40px 0" }}>
    <div style={W}>
      <SecLabel color={C.blue}>Patterns &amp; Sources</SecLabel>

      {/* ── CARD 1: WHAT PAST TOURNAMENTS TELL US ── */}
      <div style={{ marginBottom:16 }}>
        <ExpandTrigger label="What Past Tournaments Tell Us" open={tourOpen} onToggle={() => setTourOpen(o => !o)} accent={C.blue} />
      ...
      <div>
        <ExpandTrigger label="Methodology and Sources" open={methOpen} onToggle={() => setMethOpen(o => !o)} accent={C.orange} />
                ...
                <div key={i} style={{ borderBottom: i<methodologies.length-1?`1px solid ${C.border}`:"none" }}>
                  <details>
                    <summary style={{ padding:"16px 20px", fontSize:14, fontWeight:700, color:C.text, cursor:"pointer", listStyle:"none", display:"flex", justifyContent:"space-between", alignItems:"center", background:C.surface }}>
                      {m.title}
                      <svg width="10" height="10" viewBox="0 0 10 10" fill="none" style={{ flexShrink:0, marginLeft:12 }}><polygon points="2,1 9,5 2,9" fill={C.dim}/></svg>
                    </summary>
            ...
            <div>
              <button onClick={() => setSrcOpen(o => !o)} style={{ width:"100%", background:"none", border:"none", cursor:"pointer", textAlign:"left", display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom: srcOpen ? 16 : 0, padding:0 }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8, fontSize:14, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", color:C.dim }}>
                  ...
                  <div style={{ transition:"transform .3s", transform: srcOpen?"rotate(90deg)":"none" }}>
                    <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><polygon points="2,1 9,5 2,9" fill={C.dim}/></svg>
                  </div>
                </div>
              </button>
...
const SAbout = () => {
  const [open, setOpen] = useState(false);
  return (
    <section id="about" style={{ borderTop:`1px solid ${C.border}`, padding:"40px 0" }}>
      <div style={{ ...W, padding:"0 120px" }}>
        ...
        >
          <div
            className="card-blue"
            style={{ background:C.surface, borderRadius:12, padding:"28px 32px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:24 }}
          >
            <div>
              <div className="d" style={{ fontSize:"clamp(17px,2.4vw,22px)", fontWeight:700, color:C.text, lineHeight:1.2 }}>
                <span>Andy Cook: </span><span style={{ fontWeight:400 }}>Restaurant Advisor | Restaurant Technology Specialist</span>
              </div>
            </div>
            <div style={{ width:44, height:44, flexShrink:0, borderRadius:8, border:`2px solid ${open ? C.blue : C.border}`, display:"flex", alignItems:"center", justifyContent:"center", transition:"transform .3s, border-color .2s", transform:open?"rotate(45deg)":"none", color:open?C.blue:C.dim }}>
        ...
        <div className={`xbody ${open ? "open" : ""}`}>
          <div style={{ paddingBottom:0, paddingTop:24 }}>
            ...
            {/* Two-column: Andy + Harbor/Philosophy */}
            <div className="two-col" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:24, marginBottom:24 }}>

              {/* Andy */}
              <div className="card-blue" style={{ background:C.surface, borderRadius:12, padding:"32px 28px" }}>
                {/* Profile image - floated right, text wraps */}
                <div style={{ float:"right", width:110, height:110, borderRadius:"50%", overflow:"hidden", border:`2px solid ${C.blue}`, background:"transparent", marginLeft:20, marginBottom:12 }}>
                  <img src="/profile.png" alt="Andy Cook" style={{ width:"100%", height:"100%", objectFit:"cover", display:"block" }} onError={e => { e.currentTarget.style.display="none"; }} />
                </div>
                <div style={{ fontSize:14, fontWeight:700, letterSpacing:".14em", textTransform:"uppercase", color:C.dim, marginBottom:18 }}>The Author</div>
                ...
                <div style={{ fontSize:13, color:C.dim, marginBottom:24 }}>Harbor Foodservice, PNW</div>
                <p style={{ fontSize:15, color:C.muted, lineHeight:1.78, marginBottom:16 }}>
                  Independent operators are the most isolated professionals in this industry. They absorb its hardest problems personally. They rarely share them across markets. Most of the advice reaching them was built for chains and scaled down without accounting for the conditions of someone who is simultaneously the owner, the scheduler, the trainer, and the person handling a health inspection at 10 AM on a Tuesday.
                </p>
                <p style={{ fontSize:15, color:C.muted, lineHeight:1.78, marginBottom:16 }}>
                  The mature years of my career were spent working in and serving locally owned restaurants. That's where this perspective comes from, not from studying the industry from outside it, but from being inside operations that couldn't afford to get it wrong.
                </p>
                <p style={{ fontSize:15, color:C.muted, lineHeight:1.78 }}>
                  I built this platform because three tournaments produced the same pattern and the operators who needed to know about it kept getting told a stadium story that didn't apply to them. The research existed. Someone needed to put it in the right hands.
                </p>
              </div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:24 }}>
                <div style={{ background:C.surface, borderRadius:12, padding:"28px 24px 24px", border:"1px solid #4ade80", borderLeft:`4px solid #4ade80`, borderBottom:`3px solid #4ade80`, transition:"box-shadow .25s", display:"flex", flexDirection:"column" }}>
                  {/* Harbor logo - floated right, text wraps */}
                  <div style={{ float:"right", width:110, height:110, borderRadius:"50%", overflow:"hidden", border:`1px solid ${C.border}`, background:C.surf2, marginLeft:20, marginBottom:12 }}>
                    <img src="/H.png" alt="Harbor Foodservice" style={{ width:"100%", height:"100%", objectFit:"contain", display:"block" }} onError={e => { e.currentTarget.style.display="none"; }} />
                  </div>
                  <div style={{ fontSize:14, fontWeight:700, letterSpacing:".14em", textTransform:"uppercase", color:C.dim, marginBottom:14 }}>Context Worth Knowing</div>
                  <div className="d" style={{ fontSize:20, fontWeight:700, color:C.text, marginBottom:14 }}>I'm a Local Distributor Based Restaurant Advisor</div>
                  <p style={{ fontSize:15, color:C.muted, lineHeight:1.78, marginBottom:14 }}>
                    Harbor Foodservice is a locally owned distributor in the Pacific Northwest. My role at Harbor is not to sell groceries, or push a sales agenda, I help our restaurants adapt through challenges and towards opportunities. Their success is our success.
                  </p>
                  <p style={{ fontSize:15, color:C.muted, lineHeight:1.78, marginBottom:16 }}>
                    This platform operates under the Infinite Restaurants banner because the pattern it documents runs across North America, not just Harbor's geography. The separation is intentional, not evasive. I'm proud of how we serve.
                  </p>
                  <div style={{ marginTop:"auto", display:"flex", justifyContent:"flex-end" }}>
                ...
                <div className="callout-left-blue" style={{ background:C.surface, borderRadius:"0 12px 12px 0", flex:1, padding:"18px 22px 18px", borderLeft:`3px solid ${C.blue}`, display:"flex", flexDirection:"column" }}>
                  <div style={{ fontSize:14, fontWeight:700, letterSpacing:".14em", textTransform:"uppercase", color:C.dim, marginBottom:12 }}>Media Inquiries</div>
                  <p style={{ fontSize:15, color:C.muted, lineHeight:1.78, marginBottom:16 }}>
...
// ─── FOOTER ───────────────────────────────────────────────────────────────────
const Footer = () => (
  <footer style={{ borderTop:`1px solid ${C.border}`, padding:"28px 120px" }}>
    <div style={{ ...W, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
      <Logo/>
      <p style={{ fontSize:13, color:C.border }}>No vendor affiliation. No distribution agenda. No sponsored content.</p>
      <span style={{ fontSize:13, color:C.border }}>infinite restaurants 2026</span>
    </div>
  </footer>
);
...
const useActive = () => {
  ...
  useEffect(() => {
    const ids = ["who","where","position","action","about","pattern"];
    const handle = () => {
      const navH = 80;
      let current = "";
      for (const id of ids) {
