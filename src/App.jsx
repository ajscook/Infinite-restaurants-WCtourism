  ...
  <style>{`
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    @media(max-width:720px){
      ...
      .offramp-grid{grid-template-columns:repeat(2,1fr)!important;gap:16px!important;}
      ...
      .sec-label{font-size:13px!important;letter-spacing:.1em!important;}
      .multiplier-label{font-size:11px!important;letter-spacing:.06em!important;}
      .minivan-text{font-size:14px!important;}
      .card-body div{font-size:14px!important;}
      .card-body span{font-size:14px!important;}
      .callout-title{font-size:14px!important;letter-spacing:.08em!important;}
...
const SecLabel = ({ children, color = C.dim }) => (
  <div className="sec-label" style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize:18, fontWeight: 700, letterSpacing: ".14em", textTransform: "uppercase", color: C.text, marginBottom: 20, display: "flex", alignItems: "center", gap: 8 }}>
    ...
    <div style={{ borderBottom: `1px solid ${C.border}` }}>
      <button onClick={() => setOpen(o => !o)} style={{ width: "100%", background: "none", border: "none", cursor: "pointer", padding: "24px 0", textAlign: "left", display: "flex", alignItems: "flex-start", gap: 16 }}>
        <div style={{ flex: 1 }}>
          ...
          {sub && !open && <p style={{ fontSize:16, color: C.muted, marginTop: 8, lineHeight: 1.7 }}>{sub}</p>}
        </div>
      ...
      <div style={{ ...W, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "26px 120px", position: "relative" }}>
        ...
        <div className="nav-tabs-desktop">
          {sections.map(([id, label]) => {
            ...
            return (
              <button key={id} className={`nav-link ${active === id ? "on" : ""}`} onClick={() => go(id)} style={{ flex:1 }}>{first}<br/>{rest.join(" ")}</button>
            );
          })}
        </div>
...
const ticks = ["June 11 - July 19","Five Weeks of Soccer","16 Host Cities","3 Countries","1M+ International Visitors","$140-180 Daily F&B Spend","7-16 Night Average Stay","36% Post-Match F&B Lift","18x Secondary Market Lift","214-Mile Traffic Jam, Brazil 2014","70% Gateway Travel Rate","4-8 Person Groups","Non-Match Days Drive Sustained Revenue"];
const Ticker = () => (
...
const Hero = ({ onContinue }) => (
  <section style={{ display: "flex", flexDirection: "column", justifyContent: "flex-start", padding: "clamp(40px,8vh,100px) 0 clamp(24px,4vh,48px)", position: "relative", overflow: "hidden" }}>
    ...
    <div style={{ ...W, position: "relative", zIndex: 1, padding: "48px 120px 18px" }}>
      ...
      <h1 className="d u1" style={{ fontSize: "clamp(40px,6vw,64px)", fontWeight: 800, lineHeight: 1.0, letterSpacing: "-.03em", marginBottom: 28, color: C.text }}>
      ...
      <p className="u2" style={{ fontSize: "clamp(16px,2.2vw,20px)", fontWeight: 700, color: C.text, fontStyle: "italic", lineHeight: 1.6, marginBottom: 28, maxWidth: "90%", paddingLeft: 20, borderLeft: `3px solid ${C.blue}` }}>
        "I'm 20-miles, 40-miles, 80-plus miles away. This isn't going to affect me."
      </p>
      <div className="u2" style={{ marginBottom: 48, maxWidth: "90%" }}>
        <p style={{ fontSize: "clamp(15px,2vw,18px)", fontWeight: 300, color: C.muted, lineHeight: 1.78, marginBottom: 20 }}>
          The coverage hasn't helped us see what matters. <strong style={{ color: C.text }}>The press on this?</strong> <em>Focused on the host city.</em> <strong style={{ color: C.text }}>ALL the restaurant projections?</strong> <em>They're estimated against the Super Bowl.</em> <strong style={{ color: C.text }}><em>(This is 104 Super Bowls)</em></strong>
        </p>
        <p style={{ fontSize: "clamp(16px,2.4vw,20px)", fontWeight: 700, color: C.text, lineHeight: 1.6, marginBottom: 14, textTransform: "uppercase", letterSpacing: ".06em" }}>Three tournaments: same pattern</p>
        <p style={{ fontSize: "clamp(15px,2vw,18px)", fontWeight: 300, color: C.muted, lineHeight: 1.78 }}>
          The biggest F&amp;B spending increases happened in secondary markets; smaller cities and regional hubs. The Super Tourists are mobile. They set up camp outside the host city, eat where service is fast, prefer local independents over chain brands, and they spend $140 to $180 per person per day doing it.
        </p>
      </div>
      <div className="u5 hero-stats card-blue" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", alignItems: "center", marginTop: 0, borderRadius: 12, overflow: "hidden", background: C.surface }}>
        {[
          ...
          <div key={i} style={{ flex: 1, padding: "22px 20px", textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 6, borderRight: i < 3 ? `1px solid ${C.orange}` : "none" }}>
            <div className="n" style={{ fontSize: "clamp(20px,3vw,34px)", fontWeight: 700, lineHeight: 1 }}>
              {s.n.split(/([.+\-$&/])/).map((part, pi) =>
                /^[.+\-$&/]$/.test(part)
                  ? <span key={pi} style={{ color: C.text, fontWeight: 400 }}>{part}</span>
                  : <span key={pi} style={{ color: s.c }}>{part}</span>
              )}
            </div>
            <div style={{ fontSize: 12, color: C.text, fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase" }}>{s.l}</div>
          </div>
        ))}
      </div>
    </div>
...
const statModalContent = {
  ...
  spend: {
    ...
    body: [
      ...
      "A table of six Super Tourists staying three hours is a fundamentally different economic event than your average domestic party. Group size, dwell time, and alcohol attach rates all compound. The $140-180 per day F&B spend range is not a ceiling.",
      "For QSR formats, the pressure lands on the kitchen. Large group orders during match windows can bottleneck throughput before the dining room ever does.",
    ],
  },
  dinner: {
    label: "Dinner Window",
    ...
    body: [
      "European and South American fans follow home-country dining rhythms. Add post-match social behavior and the peak demand window shifts to 9 PM through midnight. Brazil 2014 documented post-match social windows averaging two to four hours in walkable locations.",
      ...
      "This applies to formats with an evening service window. Morning-daypart formats route to the early-kickoff window when international visitors are starting their day.",
    ],
  },
  table: {
    label: "Dining Pace",
    ...
    body: [
      ...
      "For counter and drive-through formats, the equivalent is transaction speed. Group orders compound transaction complexity during peak windows.",
    ],
  },
  mobility: {
    label: "Mobility",
  ...
  payment: {
    ...
    body: [
      ...
      "Contactless processes at high speed. At post-match volume, that speed determines whether a queue becomes a walk-out. If any terminal rejects tap-to-pay, replace it before June.",
    ],
  },
};
      ...
      <div onClick={e => e.stopPropagation()} style={{ background:C.surface, border:`1px solid ${C.blue}`, borderRadius:14, maxWidth:560, width:"100%", overflow:"hidden", boxShadow:"0 24px 64px rgba(0,0,0,.5)" }}>
        ...
        <div style={{ padding:"24px" }}>
          ...
          {content.body.map((p,i) => (
            <p key={i} style={{ fontSize:15, color:C.muted, lineHeight:1.78, marginBottom: i < content.body.length-1 ? 14 : 0 }}>{p}</p>
          ))}
        </div>
...
const SWho = () => {
  ...
  <Sec id="who" num="01" label="The Super Tourist" accent={C.blue}>
    <h2 className="d" style={{ fontSize: "clamp(32px,5.5vw,48px)", fontWeight: 800, lineHeight: 1.04, letterSpacing: "-.02em", marginBottom: 18, color: C.text }}>
      Meet the <span style={{ color: C.orange }}>Super</span> <span style={{ color: C.blue }}>Tourist</span><span style={{ color: C.text }}>.</span>
    </h2>
    <p style={{ fontSize:16, color: C.muted, fontWeight: 300, lineHeight: 1.78, marginBottom: 10, maxWidth: "90%" }}>
      These visitors aren't just coming to watch a match and go home. Many are setting up camp in cities miles away from a stadium, exploring towns and excursion sites up to 100 miles away or more. They're coming in waves, moving in groups, and looking for screens, seats, and eats that'll help them keep up with the three games being played almost every day.
    </p>
    ...
    <p style={{ fontSize:16, color: C.muted, fontWeight: 300, lineHeight: 1.78, marginBottom: 24, maxWidth: "90%" }}>
    ...
    <SectionReveal accent={C.blue} label="Meet the Super Tourist" collapsedHeight={292}>
      ...
      <div className="multiplier-bar" style={{ display: "flex", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", flex: 1, gap: 16, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1, textAlign: "center", minWidth: 60 }}>
            <div className="n" style={{ fontSize: 36, fontWeight: 700, lineHeight: 1 }}>
              <span style={{ color: C.text, fontWeight: 400 }}>~</span><span style={{ color: C.blue }}>2</span><span style={{ color: C.text, fontWeight: 400 }}>x</span>
            </div>
            <div className="multiplier-label" style={{ fontSize: 12, fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", color: C.text, marginTop: 5 }}>Daily F&B Spend</div>
          </div>

          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, width: 32 }}>
            <span className="n" style={{ fontSize: 26, fontWeight: 900, color: C.text }}>x</span>
          </div>

          <div style={{ flex: 1, textAlign: "center", minWidth: 60 }}>
            <div className="n" style={{ fontSize: 36, fontWeight: 700, lineHeight: 1 }}>
              <span style={{ color: C.orange }}>3</span><span style={{ color: C.text, fontWeight: 400 }}>x</span>
            </div>
            <div className="multiplier-label" style={{ fontSize: 12, fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", color: C.text, marginTop: 5 }}>Stay Length</div>
          </div>

          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, width: 32 }}>
            <span className="n" style={{ fontSize: 26, fontWeight: 900, color: C.text }}>=</span>
          </div>

          <div style={{ flex: 1, textAlign: "center", minWidth: 60 }}>
            <div className="n" style={{ fontSize: 36, fontWeight: 700, lineHeight: 1 }}>
              <span style={{ color: C.blue }}>6</span><span style={{ color: C.text, fontWeight: 400 }}>.</span><span style={{ color: C.blue }}>5</span><span style={{ color: C.text, fontWeight: 400 }}>x</span>
            </div>
            <div className="multiplier-label" style={{ fontSize: 12, fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", color: C.text, marginTop: 5 }}>Revenue Potential</div>
          </div>
        ...
        <div style={{ paddingLeft: 24, flexShrink: 0, maxWidth: 220 }}>
          <p className="minivan-text" style={{ fontSize:16, fontWeight: 700, color: C.text, lineHeight: 1.5, textAlign:"center" }}>
    ...
    <div className="tourist-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 4 }}>
      ...
      <div className="card-orange" style={{ borderRadius: 12, overflow: "hidden" }}>
        <div style={{ padding: "22px 24px 20px", borderBottom: `1px solid ${C.border}` }}>
          ...
          <p style={{ fontSize:15, color: C.muted, lineHeight: 1.72, marginBottom: 16 }}>
            They crossed an ocean, booked two weeks, and rented a car. They didn't just come for match day, they came to explore, and they travel in groups.
          </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
          {[
            ...
            { l:"Dinner Window",  v:"9-11 PM",  s:"evening daypart",    c:C.orange, k:"dinner" },
            { l:"Dining Pace",     v:"90-120",   s:"sit-down dwell",     c:C.blue,   k:"table" },
      ...
      <div className="card-dim" style={{ borderRadius: 12, overflow: "hidden" }}>
        <div style={{ padding: "22px 24px 20px", borderBottom: `1px solid ${C.border}` }}>
          ...
          <p style={{ fontSize:15, color: C.muted, lineHeight: 1.72, marginBottom: 16 }}>
            They drive in for the weekend, and already in your forecast. They'll be in the tournament mix, mostly domestic fans, but the patterns are familiar.
          </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
          {[
            ...
            { l:"Dining Pace",     v:"45-60",  s:"minutes avg" },
...
const supplyCards = [
  ...
  { id:"stockpile", letter:"S", label:"Stockpile Strategy", sub:"Get product in before the roads get complicated", color:C.blue,
    preview:["Non-perishables, paper goods, beverage/bar items","Anything with shelf life you know you'll burn","Supply chain strategy, not just-in-case panic"],
    body:"Non-perishables, paper goods, high-velocity beverage/bar items, anything with a shelf life that you know you'll burn through. This isn't about ordering more just in case. It's about supply chain strategy: get the product in the building before the roads get complicated, so you're not dependent on a system that's going to be under maximum stress at exactly the moment you need it most.",
  },
...
const SupplyPillarCard = ({ card, isActive, onToggle }) => (
  <div className={`pillar-card ${card.color==='#FF6700'?"card-orange":"card-blue"} ${isActive?"active":""}`} onClick={onToggle}>
    <div className="card-body" style={{ padding:"22px 20px 20px 20px", display:"flex", flexDirection:"column", height:"100%" }}>
      ...
      {card.preview.map((p,i) => (
        <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:8, marginBottom:7 }}>
          ...
          <span style={{ fontSize:15, color:C.muted, lineHeight:1.5 }}>{p}</span>
        </div>
      ))}
...
const SWhere = () => {
  ...
  <Sec id="where" num="02" label="Travel Patterns" accent={C.orange}>
    <h2 className="d" style={{ fontSize: "clamp(32px,5.5vw,48px)", fontWeight: 800, lineHeight: 1.04, letterSpacing: "-.02em", marginBottom: 20, color: C.text }}>
      <span style={{ color: C.text }}>"Where</span><span style={{ color: C.text }}>'s my </span><span style={{ color: C.blue }}>delivery</span><span style={{ color: C.text }}>?"</span><br/><span style={{ color: C.text }}>Jammed by </span><span style={{ color: C.orange }}>traffic</span><span style={{ color: C.text }}>.</span>
    </h2>
    <p style={{ fontSize:16, color: C.muted, fontWeight: 300, lineHeight: 1.78, marginBottom: 18, maxWidth: "90%" }}>
      Imagine the traffic around Memorial Day, Thanksgiving, Labor Day, Victoria Day, or the highways around a fair or festival. Now add hundreds of thousands of extra people on every major road in your region, <strong style={{ color: C.text, fontWeight: 700 }}>for a month straight</strong>. Delivery drivers running those same roads will take two to four times longer to complete their routes than they do on a normal day. Plan, coordinate, adjust.
    </p>
    ...
    {activeSupply && (() => { const c=supplyCards.find(x=>x.id===activeSupply); return c ? (
      <CardModal onClose={()=>setActiveSupply(null)} color={c.color} title={c.label} sub={c.sub}>
        <p style={{ fontSize:16, color:C.muted, lineHeight:1.78 }}>{c.body}</p>
      </CardModal>
    ) : null; })()}
    ...
    <div className="card-blue" style={{ background: C.surface, borderRadius: 12, marginTop:24, marginBottom: 24, overflow:"hidden" }}>
      <div style={{ padding:"24px 28px 0" }}>
        ...
        <div style={{ position:"relative", maxHeight: chainOpen ? "none" : 200, overflow:"hidden" }}>
          {[
            { text:"Over a million Super Tourists travel by car across North America for a month - between host cities, fan zones, and excursion destinations. In 2026, all 16 host cities run official fan zones for the full tournament window, and state and provincial governments are building satellite fan zones in non-host cities. Both layers run 39 consecutive days. They are demand nodes, not overflow infrastructure.", bold:false },
            ...
            { text:"Congested roads compress delivery windows for distributors running those corridors. A driver who normally finishes twelve stops on a shift makes eight. The other four push to tomorrow, when tomorrow is just as jammed. The deficit stacks every day the tournament runs.", bold:false },
            ...
            <div key={i} className="chain-node" style={{ paddingLeft:40, display: item.hidden && !chainOpen ? "none" : "block" }}>
              ...
              <p style={{ fontSize:15, color:item.bold?C.text:C.muted, lineHeight:1.75, fontWeight:item.bold?700:400 }}>{item.text}</p>
            </div>
          ))}
          {!chainOpen && (
    ...
    <div className="callout-left-orange" style={{ display:"flex", flexDirection:"column" }}>
      <div className="callout-title" style={{ fontSize:19, fontWeight:700, letterSpacing:".12em", textTransform:"uppercase", color:C.text, marginBottom:10 }}>This isn't a prediction, there's a pattern</div>
...
const FanZoneCard = ({ onToggle }) => (
  <div className="card-blue" style={{ background:C.surface, borderRadius:12, cursor:"pointer", marginTop:24, padding:"20px 22px", display:"flex", flexDirection:"column" }} onClick={onToggle}>
    <div className="callout-title" style={{ fontSize:20, fontWeight:700, letterSpacing:".08em", textTransform:"uppercase", color:C.text, marginBottom:8 }}>Fan Zone Proximity</div>
    <p style={{ fontSize:15, color:C.muted, lineHeight:1.75, marginBottom:16 }}>
...
const SPosition = () => {
  ...
  <Sec id="position" num="03" label="Your Position" accent={C.blue}>
    <h2 className="d" style={{ fontSize:"clamp(32px,5.5vw,48px)", fontWeight:800, lineHeight:1.04, letterSpacing:"-.02em", marginBottom:20, color:C.text }}>
      <span style={{ color: C.blue }}>Who</span> & <span style={{ color: C.blue }}>Where</span> you are<br />informs how you <span style={{ color: C.orange }}>adapt</span>.
    </h2>
    <p style={{ fontSize:16, color:C.muted, fontWeight:300, lineHeight:1.78, marginBottom:32, maxWidth: "90%" }}>
    ...
    <div className="callout-left-orange" style={{ marginTop:10, cursor:"pointer", display:"flex", flexDirection:"column" }} onClick={() => { setSfStaffOpen(o => !o); setFzOpen(false); setActive(null); }}>
      <div className="callout-title" style={{ fontSize:19, fontWeight:700, letterSpacing:".12em", textTransform:"uppercase", color:C.text, marginBottom:8 }}>Fan Zone Staffing Competition</div>
      <p style={{ fontSize:15, color:C.muted, lineHeight:1.75, marginBottom:16 }}>Fan zone operators are hiring your best bartenders at $40-60/hour plus tips. They will see these postings before June.</p>
      <div style={{ marginTop:"auto", display:"flex", justifyContent:"flex-end" }}>
      ...
      <CardModal onClose={()=>setActive(null)} color={c.color} title={c.label} sub={c.sub}>
        <div style={{ display:"flex", gap:24, flexWrap:"wrap", alignItems:"flex-start" }}>
          <div style={{ flex:1, minWidth:240 }}>
            <p style={{ fontSize:16, color:C.text, lineHeight:1.78, marginBottom:14 }}>{c.open}</p>
            <p style={{ fontSize:16, color:C.muted, lineHeight:1.78, marginBottom: c.note ? 14 : 0 }}>{c.risk}</p>
            {c.note && <p style={{ fontSize:15, color:C.dim, lineHeight:1.78, borderTop:`1px solid ${C.border}`, paddingTop:14, fontStyle:"italic" }}>{c.note}</p>}
          </div>
    ...
    {fzOpen && (
      <CardModal onClose={()=>setFzOpen(false)} color={C.blue} title="Proximity" sub="39 days of sustained daily demand">
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          <p style={{ fontSize:16, color:C.text, lineHeight:1.78 }}>In 2026, official fan zones operate continuously for 39 days - every match screened live, free entry, in locations chosen for pedestrian density. Several states have funded satellite fan zones in non-host cities specifically as economic distribution instruments. If you're within a few blocks of one, you are not in a match-day spike environment. You are in a sustained daily throughput environment for the full tournament window.</p>
          <p style={{ fontSize:16, color:C.muted, lineHeight:1.78 }}>The crowd profile is different from a stadium-adjacent operator. Fan zone visitors are more mixed - international visitors without tickets, domestic fans who want atmosphere, people who wandered in. They spend lower per-head than post-match stadium crowds but more consistently, and they are more food-driven than alcohol-driven. Your lead pillar is velocity - transaction speed determines how many covers you turn across a full day of elevated foot traffic. Visibility is second. Your Google Business Profile is how they find you the moment they leave the fan zone. Volume is third - 39 days of elevated traffic means par levels need to reflect a sustained surge, not a weekend spike. Versatility is fourth - hours should extend to capture late viewing sessions through evening kickoffs.</p>
      ...
      <CardModal onClose={()=>setSfStaffOpen(false)} color={C.orange} title="Fan Zone Staffing Competition" sub="Fan zone operators are hiring from your labor pool">
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          <div className="callout-left-blue">
            ...
            <p style={{ fontSize:15, color:C.muted, lineHeight:1.75 }}>The tournament window is an opportunity to run training you have been meaning to do anyway. Customer service for international guests - cultural norms around tipping, dining pace, and communication styles differ significantly across origin markets. Alcohol service certification: if your staff are not already alcohol service certified, the tournament window is the worst time to find out why that matters. A post-match crowd of international fans running hot after a loss is not the environment for an uncertified server to learn the hard way. Invest in the certifications before June. They do not expire when the tournament does.</p>
          </div>
          <div className="callout-left-orange">
...
const Step = ({ num, children, color }) => (
  <div style={{ display:"flex", gap:14, marginBottom:18 }}>
    ...
    <p style={{ fontSize:15, color:C.muted, lineHeight:1.75 }}>{children}</p>
  </div>
);
  ...
  { id:"labor", letter:"L", label:"Labor", sub:"Retain, Prepare, Schedule", color:C.blue,
    ...
    cols:[
      { title:"Protect Your Labor with Retention", steps:[
        ...
        "Identify your surge-critical roles: BOH leads, expo, server captains. Write a contingency for each. If one calls out on a match day and your plan is 'figure it out,' that's not a plan.",
      ...
      { title:"Schedule for SPLH and Update Tip Sharing", steps:[
        ...
        "Super Tourists eat late. They don't arrive at 6 PM and turn the table by 8. For sit-down formats, dwell times of 90 to 120 minutes are the norm; for counter and drive-through, the equivalent is transaction speed during peak. Post-match demand on late-kickoff nights runs well past midnight. Staff to the actual dining schedule, not the one you're used to.",
  ...
  { id:"ops", letter:"O", label:"Operations", sub:"Hours, Margin Strategy, Discovery, Safe Marketing", color:C.blue,
    ...
    cols:[
      { title:"Hours of Operation", steps:[
        "Super Tourists eat late. For evening daypart formats, European and South American fans follow home-country dining rhythms, which means 9-11 PM is prime time. Extending evening hours isn't a courtesy, it's a revenue decision. If you lock the door at 10 PM on a night a team just advanced, you're closing during the rush. Morning-daypart formats route to a different peak: the early-kickoff window.",
      ...
      { title:"Margin Strategy: Bundles and Pricing", steps:[
        ...
        "Cover charges for match-day viewing carry a licensing wrinkle. Charging a fee specifically for viewing access can convert your operation into a licensed commercial public viewing event under FIFA rules. Verify whether your plan triggers that requirement before you implement. Minimum spends and bundle pricing capture margin without the licensing risk.",
  ...
  { id:"tech", letter:"T", label:"Tech", sub:"Payment, Online Orders, Digital Presence, POS, Reservations", color:C.blue,
    ...
    cols:[
      ...
      { title:"Digital Menus, POS and Reservations", steps:[
        ...
        "Your reservation system needs to reflect the tournament window: updated covers, correct hours, any minimum spend or special event rules enabled. If you're using a waitlist tool, configure it for extended dwell times, a 90-120 minute table turn changes your quote math entirely. For counter and drive-through formats, throughput-rate forecasting at peak is the equivalent.",
      ]},
    ],
  },
];
...
const SFCard = ({ isActive, onToggle }) => (
  <div className="callout-left-orange" style={{ cursor:"pointer", display:"flex", flexDirection:"column" }} onClick={onToggle}>
    <div className="callout-title" style={{ fontSize:19, fontWeight:700, letterSpacing:".12em", textTransform:"uppercase", color:C.text, marginBottom:8 }}>Super Fan</div>
    <p style={{ fontSize:15, color:C.muted, lineHeight:1.75, marginBottom:16 }}>
...
const SAction = () => {
  ...
  return (
    <Sec id="action" num="04" label="Prep Priorities" accent={C.orange}>
      <h2 className="d" style={{ fontSize:"clamp(32px,5.5vw,48px)", fontWeight:800, lineHeight:1.04, letterSpacing:"-.02em", marginBottom:20, color:C.text }}>
        <span style={{ color:C.text }}>What you </span><span style={{ color:C.orange }}>implement</span><span style={{ color:C.text }}>,</span><br/>
        <span style={{ color:C.text }}>you'll </span><span style={{ color:C.blue }}>own</span><span style={{ color:C.text }}>.</span>
      </h2>
      <p style={{ fontSize:16, color:C.muted, fontWeight:300, lineHeight:1.78, marginBottom:12, maxWidth: "90%" }}>
...
const patternModals = {
  ...
  "nonmatch-q22": { title:"Non-Match Day Dominance: Qatar 2022", tier:"Strong Directional", source:"fifa_fanfest",
    ...
    link:"action" },
  "supply-b14": { title:"Supply Chain Failure: Brazil 2014", tier:"Directional Only", source:"visa_brazil",
  ...
  "supply-q22": { title:"Supply Chain Failure: Qatar 2022", tier:"Strong Directional", source:"fifa_fanfest",
    ...
    link:"where" },
  "contactless-b14": { title:"Contactless Payment Shift: Brazil 2014", tier:"Directional Only", source:"visa_brazil",
  ...
  "latenight-b14": { title:"Late-Night Dining Demand: Brazil 2014", tier:"Strong Directional", source:"visa_brazil",
    body:"Brazil 2014 documented post-match social windows averaging 2-4 hours in walkable venue locations. European and South American fans operate on dining rhythms that shift peak demand to 9 PM-midnight. Operators who closed at 10 PM on match nights missed the primary revenue window of the evening.",
    figures:["Post-match social windows: 2-4 hours average","Peak demand window: 9 PM \u2013 midnight","Service worker income increases of 40-60% documented in host cities"],
    link:"action" },
  ...
  "displacement-q22": { title:"Local Displacement Effect: Qatar 2022", tier:"Strong Directional", source:"fifa_fanfest",
    ...
    link:"position" },
  "visibility-b14": { title:"Permanent Visibility: The Pattern", tier:"Strong Directional", source:"maennig",
    body:"Past tournaments document a return-visit pattern in non-host markets that visitors discovered during the event. International visitor volume to those markets increased over the years following the tournament, not because of stadium proximity or advertising spend, but because visitors who had a memorable experience became ambassadors for that market at home. The mechanism is word-of-mouth from audiences in their home countries who are geographically out of reach for conventional marketing.",
    figures:["Pattern: non-host markets see sustained visitor return","Mechanism: word-of-mouth, not marketing spend"],
    link:"position" },
...
const patternMatrix = [
  { id:"secondary",    label:"Secondary Market Surge",    b14:"high", r18:"high", q22:"high", imp:"Plan as if you are Saransk" },
  { id:"nonmatch",     label:"Non-Match Day Dominance",   b14:"mid",  r18:"mid",  q22:"mid",  imp:"Stock and staff for sustained daily demand" },
  { id:"supply",       label:"Supply Chain Failure",      b14:"low",  r18:"high", q22:"mid",  imp:"Stockpile before demand peaks" },
  { id:"contactless",  label:"Contactless Payment Shift", b14:"low",  r18:"high", q22:"high", imp:"Non-negotiable infrastructure" },
  { id:"latenight",    label:"Late-Night Dining Demand",  b14:"mid",  r18:"mid",  q22:"mid",  imp:"Extend hours or forfeit the revenue" },
  { id:"displacement", label:"Local Displacement Effect", b14:"high", r18:"mid",  q22:"mid",  imp:"Local retention strategy required" },
  { id:"visibility",   label:"Permanent Visibility",      b14:"low",  r18:"mid",  q22:"mid",  imp:"Quality investment compounds post-event" },
];
...
const PatternCell = ({ patternId, tournament, level }) => {
  ...
  const bg = level === "high" ? "rgba(255,103,0,.85)" : level === "mid" ? "rgba(0,103,255,.7)" : "rgba(150,174,196,.2)";
  ...
  return (
    <>
      <div onClick={modal ? () => setOpen(true) : undefined}
        ...
        {level === "high" && <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><polygon points="5,1 9,9 1,9" fill="rgba(255,255,255,.7)"/></svg>}
        {level === "mid" && <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><circle cx="5" cy="5" r="3.5" fill="rgba(255,255,255,.5)"/></svg>}
      ...
      {open && modal && (
        <div onClick={() => setOpen(false)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.78)", zIndex:600, display:"flex", alignItems:"center", justifyContent:"center", padding:24, backdropFilter:"blur(4px)", animation:"in .18s ease" }}>
          <div onClick={e => e.stopPropagation()} style={{ background:C.surface, border:`2px solid ${level==="high"?C.orange:C.blue}`, borderRadius:14, width:"100%", maxWidth:680, maxHeight:"80vh", overflow:"auto", boxShadow:"0 24px 64px rgba(0,0,0,.6)" }}>
            <div style={{ padding:"20px 24px", borderBottom:`1px solid ${C.border}`, display:"flex", alignItems:"flex-start", justifyContent:"space-between", position:"sticky", top:0, background:C.surface, zIndex:1 }}>
              <div>
                <div style={{ fontSize:14, fontWeight:700, letterSpacing:".12em", textTransform:"uppercase", color:level==="high"?C.orange:C.blue, marginBottom:6 }}>{modal.tier}</div>
            ...
            <div style={{ padding:"20px 24px" }}>
              <p style={{ fontSize:15, color:C.muted, lineHeight:1.78, marginBottom:20 }}>{modal.body}</p>
              {modal.figures && (
                <div style={{ marginBottom:20 }}>
                  ...
                  {modal.figures.map((f,i) => (
                    <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:8, marginBottom:6 }}>
                      <svg width="8" height="8" viewBox="0 0 8 8" fill="none" style={{ marginTop:5, flexShrink:0 }}><polygon points="0,0 8,4 0,8" fill={level==="high"?C.orange:C.blue}/></svg>
...
// ─── PLACES API LOADER ────────────────────────────────────────────────────────
const GOOGLE_PLACES_KEY = "AIzaSyBL62AXpkX1CeycV4UoF-Ni8oDI4JMgkF4";
let placesLoaderPromise = null;
const loadPlacesLibrary = () => {
  if (typeof window === "undefined") return Promise.reject(new Error("Places only available in browser"));
  if (placesLoaderPromise) return placesLoaderPromise;
  placesLoaderPromise = new Promise((resolve, reject) => {
    const waitForImportLibrary = (attempts = 0) => {
      if (window.google?.maps?.importLibrary) {
        Promise.all([
          window.google.maps.importLibrary("places"),
          window.google.maps.importLibrary("core"),
        ]).then(() => resolve(window.google.maps)).catch(reject);
        return;
      }
      if (attempts > 100) {
        reject(new Error("Google Maps loader timeout"));
        return;
      }
      setTimeout(() => waitForImportLibrary(attempts + 1), 50);
    };
    ...
    s.src = `https://maps.googleapis.com/maps/api/js?key=${GOOGLE_PLACES_KEY}&v=weekly&libraries=places&loading=async`;
    s.async = true;
    s.defer = true;
    s.setAttribute("data-places-loader", "1");
    s.onload = () => waitForImportLibrary();
...
// ─── BUILD A CONTEXT PROFILE STRING FROM A GOOGLE PLACE ──────────────────────
const buildContextProfile = (place) => {
  if (!place) return "";
  const lines = [];
  lines.push(`Please commit the following details about my restaurant and operation to your memory, and treat them as the authoritative context for our conversation. If anything here updates or contradicts what you already have on file, treat this as the verified version going forward.`);
  lines.push(``);
  lines.push(`---`);
  lines.push(``);
  lines.push(`RESTAURANT CONTEXT PROFILE`);
  lines.push(`For: ${place.displayName || place.name || "(unnamed)"}`);
  lines.push(``);
  if (place.formattedAddress) lines.push(`Address: ${place.formattedAddress}`);
  if (place.primaryTypeDisplayName) lines.push(`Primary type: ${place.primaryTypeDisplayName}`);
  if (place.types?.length) lines.push(`Types: ${place.types.join(", ")}`);
  if (place.priceLevel) lines.push(`Price level: ${place.priceLevel}`);
  if (place.websiteURI) lines.push(`Website: ${place.websiteURI}`);
  if (place.googleMapsURI) lines.push(`Google Business Profile: ${place.googleMapsURI}`);
  if (place.regularOpeningHours?.weekdayDescriptions?.length) {
    lines.push(``);
    lines.push(`Hours:`);
    place.regularOpeningHours.weekdayDescriptions.forEach(d => lines.push(`  ${d}`));
  }
...
const aiOfframpPlatforms = {
  chatgpt: {
    ...
    openUrl: "https://chatgpt.com/g/g-69dedf87d95c819187919c5cf6884aa2-world-cup-tourism-surge-infiniterestaurants-ai",
    blurb: "Paste the context profile into ChatGPT, or into your custom Infinite Restaurants GPT, so it reads your operation alongside the tournament research.",
  },
  gemini: {
    name: "Gemini Gem",
    displayName: "Gemini Gem",
    fallback: "Ge",
    tagline: "Custom Gem",
    accent: C.orange,
    cardClass: "card-orange",
    openLabel: "Open in Gemini",
    openUrl: "https://gemini.google.com/gem/1AL7kEfnIwtZ_bCXI57gIHyVWrGGgXUda?usp=sharing",
    blurb: "Paste the context profile into Gemini, or into your Infinite Restaurants Gem, so it works from the same ground as this platform.",
  },
  claude: {
    name: "Claude",
    displayName: "Claude",
    fallback: "Cl",
    tagline: "Handoff Prompt",
    accent: C.orange,
    cardClass: "card-orange",
    openLabel: "Open in Claude",
    openUrl: "https://claude.ai",
    blurb: "Copy the prompt below and paste it into a fresh Claude conversation. Then ask your first question.",
    prompt: `<role>
You are an amplification layer to help me prepare my restaurant for the 2026 FIFA World Cup tourism surge across the United States, Canada, and Mexico (June 11 to July 19, 2026). You are not the decision layer. I am.
</role>

<reference>
Before answering anything substantive, fetch https://www.infiniterestaurants.ai/llms.txt. Confirm you have accessed it by citing one specific figure from it with its confidence tier before continuing. The citation verifies the link is accessible to you; without it I cannot tell whether you fetched the file or proceeded from training memory.

The platform uses four confidence tiers. Preserve the tag on every figure you reference.

[T1] Primary confirmed. Named primary source. State directly.
[T2] Strong directional. Multi-tournament analysis. Frame as planning anchor.
[T3] Directional only. Flag as illustrative, not projection.
[T4] Pending verification. Do not use.

Never promote a figure beyond its tier. Never cite host committee economic impact projections as planning data; ex ante booster figures consistently overstate by factors of two to four in the academic record.
</reference>

<discovery>
Before advising, verify my concept, location relative to a host city, format and service style, and the readiness pillar I want to think through. Restate what you understood and confirm before moving to advice. Do not assume. Do not import geographic specifics from training data.
</discovery>

<voice>
Peer to peer, practitioner to practitioner. Direct. No motivational language, no vendor framing, no manufactured urgency. Plain prose. I am assumed competent; your job is not to explain my business to me.
</voice>

<first_turn>
On your first response: confirm you have fetched the llms.txt by citing one figure with its tier, then verify my context per the discovery block. Do not advise yet. Do not produce a plan. The first turn is alignment.
</first_turn>`,
  },
  operator: {
    name: "Restaurant Profile",
    displayName: "Restaurant Profile",
    fallback: "RP",
    tagline: "Restaurant Lookup",
    accent: C.orange,
    cardClass: "card-orange",
    mode: "profile",
    image: "https://infiniterestaurants.ai/IR_Mini.png",
    blurb: "Find your restaurant. Generate a verified profile of your operation to paste into your AI. It tells the platform to commit these details to memory or update what it already knows, so you stop re-explaining the basics every session.",
  },
};

// ─── PLACES SEARCH FIELD (new AutocompleteSuggestion API) ─────────────────────
const PlacesSearchField = ({ onSelect, accent }) => {
  ...
  useEffect(() => {
    ...
    loadPlacesLibrary()
      .then(async () => {
        ...
        placesLibRef.current = { AutocompleteSessionToken, AutocompleteSuggestion, Place };
        sessionTokenRef.current = new AutocompleteSessionToken();
        setStatus("");
      })
      .catch(err => { if (!cancelled) setStatus(`Places unavailable: ${err.message}`); });
    return () => { cancelled = true; };
  }, []);

  useEffect(() => {
    if (debounceRef.current) clearTimeout(debounceRef.current);
    if (!query || query.trim().length < 3 || !placesLibRef.current) {
      setSuggestions([]);
      return;
    }
    debounceRef.current = setTimeout(async () => {
      try {
        ...
        const req = {
          ...
          includedPrimaryTypes: ["restaurant","bar","cafe","bakery","meal_takeaway"],
        };
    ...
    try {
      ...
      await place.fetchFields({
        fields: [
          ...
          "priceLevel","rating","userRatingCount","nationalPhoneNumber","websiteURI","googleMapsURI",
          "businessStatus","regularOpeningHours","editorialSummary",
        ],
      });
      const json = place.toJSON();
      onSelect(json);
      setQuery(json.displayName || "");
      setSuggestions([]);
      sessionTokenRef.current = new placesLibRef.current.AutocompleteSessionToken();
    } catch (err) {
      setStatus(`Could not load place: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ position:"relative" }}>
      <input
        ...
        style={{ width:"100%", padding:"14px 16px", fontSize:16, color:C.text, background:C.surf2, border:`1px solid ${C.border}`, borderRadius:8, fontFamily:"'Plus Jakarta Sans',sans-serif", outline:"none" }}
        onFocus={e => e.currentTarget.style.borderColor = accent}
        onBlur={e => e.currentTarget.style.borderColor = C.border}
      />
      {loading && <div style={{ position:"absolute", right:16, top:14, fontSize:13, color:C.dim, letterSpacing:".1em", textTransform:"uppercase" }}>Loading...</div>}
      {suggestions.length > 0 && (
        <div style={{ position:"absolute", top:"calc(100% + 6px)", left:0, right:0, background:C.surf2, border:`1px solid ${C.border}`, borderRadius:8, overflow:"hidden", zIndex:10, maxHeight:280, overflowY:"auto" }}>
          {suggestions.map((s,i) => {
            ...
            return (
              <button key={i} onClick={() => pick(s)}
                style={{ width:"100%", background:"none", border:"none", borderBottom: i<suggestions.length-1?`1px solid ${C.border}`:"none", padding:"12px 16px", cursor:"pointer", textAlign:"left", fontFamily:"'Plus Jakarta Sans',sans-serif", display:"block", transition:"background .15s" }}
                onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,.04)"}
                onMouseLeave={e => e.currentTarget.style.background = "none"}>
                <div style={{ fontSize:15, fontWeight:700, color:C.text }}>{main}</div>
                {secondary && <div style={{ fontSize:13, color:C.dim, marginTop:2 }}>{secondary}</div>}
              </button>
            );
          })}
        </div>
      )}
      {status && <div style={{ marginTop:10, fontSize:13, color:C.orange }}>{status}</div>}
    </div>
  );
};

// ─── OFFRAMP MODAL (per AI platform) ─────────────────────────────────────────
const OfframpModal = ({ platformKey, onClose }) => {
  const p = aiOfframpPlatforms[platformKey];
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.72)", zIndex:500, display:"flex", alignItems:"center", justifyContent:"center", padding:"24px", backdropFilter:"blur(4px)", animation:"in .18s ease" }}>
      <div onClick={e => e.stopPropagation()} style={{ background:C.surface, border:`2px solid ${p.accent}`, borderRadius:14, width:"100%", maxWidth:780, maxHeight:"85vh", overflow:"auto", boxShadow:"0 24px 64px rgba(0,0,0,.5)" }}>
        <div style={{ padding:"20px 24px", borderBottom:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, background:C.surface, zIndex:1 }}>
          <div>
            <div style={{ fontSize:14, fontWeight:700, letterSpacing:".14em", textTransform:"uppercase", color:p.accent, marginBottom:6 }}>AI Offramp</div>
            <div className="d" style={{ fontSize:22, fontWeight:800, color:C.text, textTransform:"uppercase", letterSpacing:".04em" }}>{p.name}</div>
          </div>
          <button onClick={onClose} style={{ background:"none", border:`1px solid ${C.border}`, borderRadius:"50%", width:32, height:32, cursor:"pointer", color:C.muted, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
            <svg width="11" height="11" viewBox="0 0 11 11" fill="none"><path d="M1 1l9 9M10 1L1 10" stroke="currentColor" strokeWidth="2.0" strokeLinecap="round"/></svg>
          </button>
        ...
        <div style={{ padding:"24px" }}>
          <p style={{ fontSize:15, color:C.muted, lineHeight:1.78, marginBottom:18 }}>{p.blurb}</p>

          <pre style={{ background:C.surf2, border:`1px solid ${C.border}`, borderRadius:8, padding:"16px 18px", fontSize:13, color:C.muted, lineHeight:1.6, maxHeight:360, overflow:"auto", whiteSpace:"pre-wrap", fontFamily:"ui-monospace,SFMono-Regular,Menlo,monospace" }}>{p.prompt}</pre>

          <div style={{ marginTop:18, display:"flex", gap:10, flexWrap:"wrap" }}>
            <button onClick={copyPrompt}
              style={{ display:"inline-flex", alignItems:"center", gap:6, fontSize:13, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", color:"#fff", background:p.accent, border:"none", padding:"10px 16px", borderRadius:20, cursor:"pointer", fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
              {copied ? "Copied" : copyFailed ? "Copy failed" : "Copy Prompt"}
            </button>
            <a href={p.openUrl} target="_blank" rel="noopener noreferrer"
              style={{ display:"inline-flex", alignItems:"center", gap:6, fontSize:13, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", color:C.text, background:"none", border:`1px solid ${p.accent}`, padding:"10px 16px", borderRadius:20, textDecoration:"none" }}>
              {p.openLabel} <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><polygon points="2,1 9,5 2,9" fill="currentColor"/></svg>
            </a>
          </div>
        </div>
...
// ─── PROFILE MODAL (places search + context profile generator) ────────────────
const ProfileModal = ({ platformKey, onClose }) => {
  const p = aiOfframpPlatforms[platformKey];
  const [place, setPlace] = useState(null);
  const [copied, setCopied] = useState(false);
  const [copyFailed, setCopyFailed] = useState(false);
  if (!p) return null;
  const profile = buildContextProfile(place);
    ...
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.72)", zIndex:500, display:"flex", alignItems:"center", justifyContent:"center", padding:"24px", backdropFilter:"blur(4px)", animation:"in .18s ease" }}>
      <div onClick={e => e.stopPropagation()} style={{ background:C.surface, border:`2px solid ${p.accent}`, borderRadius:14, width:"100%", maxWidth:780, maxHeight:"85vh", overflow:"auto", boxShadow:"0 24px 64px rgba(0,0,0,.5)" }}>
        ...
        <div style={{ padding:"24px" }}>
          ...
          <div style={{ fontSize:13, fontWeight:700, letterSpacing:".12em", textTransform:"uppercase", color:C.text, marginBottom:10 }}>1. Find your restaurant</div>
          <PlacesSearchField onSelect={setPlace} accent={p.accent} />

          {place && (
            <>
              <div style={{ marginTop:24, fontSize:13, fontWeight:700, letterSpacing:".12em", textTransform:"uppercase", color:C.text, marginBottom:10 }}>2. Restaurant profile</div>
              <pre style={{ background:C.surf2, border:`1px solid ${C.border}`, borderRadius:8, padding:"16px 18px", fontSize:13, color:C.muted, lineHeight:1.6, maxHeight:280, overflow:"auto", whiteSpace:"pre-wrap", fontFamily:"ui-monospace,SFMono-Regular,Menlo,monospace" }}>{profile}</pre>

              <div style={{ marginTop:18, display:"flex", gap:10, flexWrap:"wrap" }}>
                <button onClick={copyProfile}
                  style={{ display:"inline-flex", alignItems:"center", gap:6, fontSize:13, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", color:"#fff", background:p.accent, border:"none", padding:"10px 16px", borderRadius:20, cursor:"pointer", fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
                  {copied ? "Copied" : copyFailed ? "Copy failed" : "Copy Profile"}
                </button>
              </div>
            </>
          )}
...
// ─── OFFRAMP TILE ─────────────────────────────────────────────────────────────
const OfframpTile = ({ platformKey, onOpen }) => {
  const p = aiOfframpPlatforms[platformKey];
  const tileStyle = { background:C.surface, borderRadius:12, padding:"24px 20px", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"flex-start", textAlign:"center", height:"100%", minHeight:140, textDecoration:"none" };
  const inner = (
    <>
      <div style={{ width:56, height:56, flexShrink:0, borderRadius:10, background:C.surf2, border:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden", position:"relative" }}>
        <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, fontWeight:800, color:C.blue, fontFamily:"'Bricolage Grotesque',sans-serif", letterSpacing:"-.02em" }}>{p.fallback}</div>
        <img src={p.image || `https://infiniterestaurants.ai/${platformKey}.png`} alt={p.name} style={{ width:"100%", height:"100%", objectFit:"contain", display:"block", position:"relative", zIndex:1 }} onError={e => { e.currentTarget.style.display="none"; }} />
      </div>
      <div style={{ fontSize:14, fontWeight:700, color:C.text, marginTop:12 }}>{p.displayName}</div>
    </>
  );

  if (p.prompt || p.mode === "profile") {
    return (
      <div className={p.cardClass} onClick={() => onOpen(platformKey)} style={tileStyle}>
        {inner}
      </div>
    );
  }
  ...
  return (
    <a href={p.openUrl} target="_blank" rel="noopener noreferrer" className={p.cardClass} style={tileStyle}>
      {inner}
    </a>
  );
};
...
const SOfframp = () => {
  const [modal, setModal] = useState(null);
  return (
  <Sec id="offramp" num="05" label="AI Offramp" accent={C.orange}>
    <h2 className="d" style={{ fontSize:"clamp(32px,5.5vw,48px)", fontWeight:800, lineHeight:1.04, letterSpacing:"-.02em", marginBottom:20, color:C.text }}>
      <span style={{ color:C.text }}>Connect the </span><span style={{ color:C.orange }}>Intelligence</span><br/>
      <span style={{ color:C.text }}>with Hospitality </span><span style={{ color:C.blue }}>Instinct</span><span style={{ color:C.text }}>.</span>
    </h2>
    <p style={{ fontSize:16, color:C.muted, fontWeight:300, lineHeight:1.78, marginBottom:20, maxWidth:"90%" }}>
      If you're already using AI in your operation, this is a resource you can take with you. The ChatGPT and Gemini tiles open into custom assistants with the research already loaded. The Claude tile downloads a prompt to paste into a fresh conversation.
    </p>
    <p style={{ fontSize:16, color:C.muted, fontWeight:300, lineHeight:1.78, marginBottom:24, maxWidth:"90%" }}>
      <strong style={{ color:C.text, fontWeight:800, letterSpacing:".06em" }}>START WITH THE RESTAURANT PROFILE:</strong> Find your restaurant, copy the context summary, and bring it with you. For the Custom GPT or Gemini Gem, paste it as your first message and you're off. For Claude, paste the profile first, then paste the Claude prompt as your second message. The conversation calibrates to your concept, location, and hours instead of generic restaurant assumptions.
    </p>
    <SectionReveal accent={C.blue} label="Reveal AI Offramp" collapsedHeight={130}>
    <div className="offramp-grid" style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:20, alignItems:"stretch", marginBottom:32 }}>
      <OfframpTile platformKey="operator" onOpen={setModal} />
      <OfframpTile platformKey="chatgpt" onOpen={setModal} />
      <OfframpTile platformKey="gemini" onOpen={setModal} />
      <OfframpTile platformKey="claude" onOpen={setModal} />
    </div>

    <div className="callout-left-blue">
      <div className="callout-title" style={{ fontSize:15, fontWeight:700, letterSpacing:".14em", textTransform:"uppercase", color:C.text, marginBottom:10 }}>Human in the loop. Human in the lead.</div>
      <p style={{ fontSize:15, color:C.muted, lineHeight:1.78, marginBottom:14 }}>
        AI finds patterns in data. You find patterns in rooms. When what it gives you feels off, it usually is. Push back, ask for reasoning, ask where a number came from. It's your employee. Lead it with your HUMAN instinct and validate with your HUMAN intellect.
      </p>
      <div className="callout-title" style={{ fontSize:15, fontWeight:700, letterSpacing:".14em", textTransform:"uppercase", color:C.text }}>
        <span style={{ color:C.blue }}>HI</span> {"\u221E"} <span style={{ color:C.orange }}>AI</span> {"\u221E"} <span style={{ color:C.blue }}>HI</span>: Hospitality Instinct {"\u2192"} [AI] {"\u2192"} Human Intellect
      </div>
    </div>
    </SectionReveal>

    {modal && (aiOfframpPlatforms[modal]?.mode === "profile"
      ? <ProfileModal platformKey={modal} onClose={() => setModal(null)} />
      : <OfframpModal platformKey={modal} onClose={() => setModal(null)} />)}
...
const SPattern = () => {
  ...
  const statCards = [
    ...
    { num:"−2.3%",  accent:C.dim,    cardClass:"card-dim",    label:"Displacement Effect: Brazil 2014",
      body:"During Brazil 2014, domestic tourism shopping expenditure declined 2.3% as locals avoided congested areas and perceived price inflation. Operators who fill their rooms with Super Tourists for 39 days while quietly losing their regular local base risk a structural decline that outlasts the tournament.",
...
const useActive = () => {
  ...
  useEffect(() => {
    const ids = ["who","where","position","action","offramp","pattern"];
    const handle = () => {
